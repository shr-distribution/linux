...
#include <linux/delay.h>
#include "panel-z10-eink-i2c.h"
#include "panel-z10-eink-cpld-fw.h"


#ifndef CONFIG_FPGA_EARLY_PORTING
#define I2C_I2C_LCD_BIAS_CHANNEL 0
#define TPS_I2C_BUSNUM  I2C_I2C_LCD_BIAS_CHANNEL	/* for I2C channel 0 */
#define I2C_ID_NAME "eink_cpld"
#define TPS_ADDR 0x3E


#if defined(CONFIG_MTK_LEGACY)
static struct i2c_board_info eink_cpld_board_info __initdata = { I2C_BOARD_INFO(I2C_ID_NAME, TPS_ADDR) };
#endif
#if !defined(CONFIG_MTK_LEGACY)
...
/*static struct i2c_client *eink_cpld_i2c_client;*/
struct i2c_client *eink_cpld_i2c_client;
int eink_cpld_read_bytes(unsigned char addr, unsigned char *returnData);
int eink_cpld_write_bytes(unsigned char addr, unsigned char value);

/*****************************************************************************
 * Function Prototype
 *****************************************************************************/
static int eink_cpld_probe(struct i2c_client *client, const struct i2c_device_id *id);
static int eink_cpld_remove(struct i2c_client *client);
...
const unsigned char lastest_fw_version = 0x36;								//每次必须手动修改这个值，这个值由CPLD出固件人员告知
...
void check_need_upgrade(void)
{
	...
	eink_cpld_read_bytes(0x80, &cpld_reg_fw_version);						//read fw version
	printk("fpga read reg fw version is %x\n",cpld_reg_fw_version);

	if ((cpld_reg_fw_version != 0x25) && (cpld_reg_fw_version < lastest_fw_version))
	{
...
static const struct i2c_device_id eink_cpld_id[] = {
	{I2C_ID_NAME, 0},
	{}
};

static struct i2c_driver eink_cpld_iic_driver = {
	.id_table = eink_cpld_id,
	.probe = eink_cpld_probe,
	.remove = eink_cpld_remove,
			   ...
			   char *buf)
{
	...
	eink_cpld_read_bytes(0x8A, &data);			//read vcom value
...
static ssize_t
ntc_show_registers(struct device *dev, struct device_attribute *attr,
			   char *buf)
{

	unsigned char data = 0;

	eink_cpld_read_bytes(0x8F, &data);			//read NTC value
	printk("ntc_show_registers enter, fpga read ntc value is %x\n",data);
...
static ssize_t
eink_cpld_show_registers(struct device *dev, struct device_attribute *attr,
			   char *buf)
{

	unsigned char data = 0;

	eink_cpld_read_bytes(0x80, &data);			//read fw version
	printk("eink_cpld_show_registers enter, fpga read fw version is %x\n",data);
...
static ssize_t
eink_cpld_store_registers(struct device *dev,
			...
			size_t count)
{
	...
	printk("eink_cpld_store_registers val is %d\n",val);
	if(val ==1)
	{
		eink_cpld_write_bytes(0x8C, 0x00);			//test for write---fpga
		refresh_mode = 0;
		printk("eink_cpld_store_registers11111 enter\n");
	}
	else if(val == 2)
	{
		eink_cpld_write_bytes(0x8C, 0x01);			//test for write---fpga
		refresh_mode = 1;
		printk("eink_cpld_store_registers22222 enter\n");
	}
	else if(val ==3)
	{
		eink_cpld_write_bytes(0x8C, 0x02);			//test for write---fpga
		refresh_mode = 2;
		printk("eink_cpld_store_registers33333 enter\n");
	}
	else if(val ==4)
	{
		eink_cpld_write_bytes(0x8C, 0x03);			//test for write---fpga
		refresh_mode = 3;
		printk("eink_cpld_store_registers44444 enter\n");
	}

	else if(val ==5)
	{
		printk("eink_cpld_store_registers55555 enter\n");
		eink_cpld_write_bytes(0x8E, 0x05);			//test for write---fpga
	}

	else if(val == 6)								//read cpld id
 	{
		eink_cpld_read_bytes(0x88, &data);			//test for read---fpga
		printk("fpga read cpld id is %x\n",data);
	}

	else if(val == 7)								//read fw version
 	{
		eink_cpld_read_bytes(0x80, &data);			//test for read---fpga
 	...
 	{
		printk("eink_cpld_store_registers9999 enter moshaoxi1111\n");
		update_firmware();
		printk("eink_cpld_store_registers9999 exit moshaoxi222\n");
	}
	...
	//TEST for read---fpga
 	...
 	{
		eink_cpld_read_bytes(0x84, &data);			//test for read---fpga
		printk("fpga read waveform version is %d\n",data);
	}

	else if(val == 11)								//read display switch
 	{
		eink_cpld_read_bytes(0x89, &data);			//test for read---fpga
		printk("fpga read display switch is %d\n",data);
	}

	else if(val == 12)								//read VCOM
 	{
		eink_cpld_read_bytes(0x8A, &data);			//test for read---fpga
		printk("fpga read VCOM is %d\n",data);
	}

	else if(val == 13)								//read CLEAN
 	{
		eink_cpld_read_bytes(0x8B, &data);			//test for read---fpga
		printk("fpga read CLEAN is %d\n",data);
	}

	else if(val == 14)								//read DISPLAY MODE
 	{
		eink_cpld_read_bytes(0x8C, &data);			//test for read---fpga
		printk("fpga read DISPLAY MODE is %d\n",data);
	}

	else if(val == 15)								//read REFRESH
 	{
		eink_cpld_read_bytes(0x8D, &data);			//test for read---fpga
		printk("fpga read REFRESH is %d\n",data);
	}

	else if(val == 16)								//read REFRESH
 	{
		ack = pango_PGC_I2C_WriteFeatureControlRegister(dev_cpld, 0x400383C1); 
		if (!ack)
		{
	...
	{
		//1.读特征控制位准备
		fpga_send_cmd_only(PGC_I2C_WREN);
		pango_PGC_I2C_WakeupEmbedFlash();
		//2.读特征控制位
		ack = fpga_read_ctl_cmd(buff);
		if (ack < 0)
		{
			printk("I2C bus error: Fail to write command %x with ack 0!\n", PGC_I2C_READ_CTL);
		}
	}
	printk("eink_cpld_store_registers exit\n");
	return count;
}
...
static int firmware_load(unsigned char *firmeware, unsigned int *firmeware_length)
{
    ...
    ret = request_firmware(&firm_data_bin, FPGA_FW_BIN_PATH, &eink_cpld_i2c_client->dev);
...
static DEVICE_ATTR(eink_cpld_registers, S_IRUGO | S_IWUSR, eink_cpld_show_registers,
		   eink_cpld_store_registers);
...
static DEVICE_ATTR(ntc_registers, S_IRUGO | S_IWUSR, ntc_show_registers,
		   NULL);
		   
static struct attribute *eink_cpld_attributes[] = {
	&dev_attr_eink_cpld_registers.attr,
	&dev_attr_vcom_registers.attr,
	&dev_attr_refresh_mode.attr,
	&dev_attr_ntc_registers.attr,
	NULL,
};

static const struct attribute_group eink_cpld_attr_group = {
	.attrs = eink_cpld_attributes,
};

static int eink_cpld_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	int ret;
	struct eink_cpld_i2c_dev *i2c_dev;

	pr_info("moshaoxi eink_cpld_iic_probe\n");
	pr_info("TPS: info==>name=%s addr=0x%x\n", client->name, client->addr);

	i2c_dev = devm_kzalloc(&client->dev, sizeof(struct eink_cpld_i2c_dev), GFP_KERNEL);
	if (!i2c_dev)
		return -ENOMEM;

	i2c_dev->dev = &client->dev;
	eink_cpld_i2c_client = client;

	ret = sysfs_create_group(&i2c_dev->dev->kobj, &eink_cpld_attr_group);
    if (ret < 0)
	{
        printk("eink_cpld attr group create failed\n");
	}
    else
	{
        printk("eink_cpld attr group create success!\n");
	}
...
static int eink_cpld_remove(struct i2c_client *client)
{
	pr_info("eink_cpld_remove\n");
	eink_cpld_i2c_client = NULL;
...
{
	int ret = 0;
	struct i2c_client *client = eink_cpld_i2c_client;
	char write_data[1] = { 0 };
	...
	printk("fpga_send_val_only val = 0x%x \n", val);
	return ret;
}
...
int fpga_send_cmd_recive_ops3(unsigned char cmd, unsigned char ops,unsigned int *returnData)
{
	...
	printk("fpga_send_cmd_recive_ops3 start!!\n");
...
int fpga_read_ctl_cmd(unsigned char *buf)			//MSX12345
{
	...
	printk("fpga_read_ctl_cmd start!!\n");
	...
	if (ret < 0)
		printk("fpga write PGC_I2C_WREN fail !!\n");
...
int fpga_program_ctl_cmd(unsigned int feature_control_val)			//MSX1234
{
	...
	write_data[4]= (feature_control_val >> 0 ) & 0xFF;
	write_data[5]= (feature_control_val >> 8 ) & 0xFF;
	write_data[6]= (feature_control_val >> 16 ) & 0xFF;
	write_data[7]= (feature_control_val >> 24 ) & 0xFF;
	printk("write_data:0x%x 0x%x 0x%x 0x%x 0x%x 0x%x 0x%x 0x%x\n", write_data[0], write_data[1], write_data[2], write_data[3], write_data[4], write_data[5], write_data[6], write_data[7]);
...
/*static int eink_cpld_write_bytes(unsigned char addr, unsigned char value)*/
int eink_cpld_write_bytes(unsigned char addr, unsigned char value)
{
	int ret = 0;
	struct i2c_client *client = eink_cpld_i2c_client;
	...
	ret = i2c_master_send(client, write_data, 2);
	if (ret < 0)
		pr_info("eink_cpld write data fail !!\n");
	printk("%s: addr:0x%x, value:0x%x\n", __func__, addr, value);
	return ret;
}
EXPORT_SYMBOL(eink_cpld_write_bytes);
...
EXPORT_SYMBOL(eink_cpld_read_bytes);

static int __init eink_cpld_iic_init(void)
{
	pr_info("%s\n", __func__);
#if defined(CONFIG_MTK_LEGACY)
	i2c_register_board_info(TPS_I2C_BUSNUM, &eink_cpld_board_info, 1);
#endif
	pr_info("eink_cpld_iic_init2\n");
	i2c_add_driver(&eink_cpld_iic_driver);
	pr_info("%s success\n", __func__);
	return 0;
}

static void __exit eink_cpld_iic_exit(void)
{
	pr_info("%s\n", __func__);
	i2c_del_driver(&eink_cpld_iic_driver);
}


module_init(eink_cpld_iic_init);
module_exit(eink_cpld_iic_exit);

MODULE_AUTHOR("Kaiduan Cao <kaiduan.cao@mediatek.com>");
MODULE_DESCRIPTION("MTK eink_cpld I2C Driver");
MODULE_LICENSE("GPL");
#endif
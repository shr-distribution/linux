...
//msx-add
	ctx->v18_en = devm_gpiod_get(ctx->dev, "v18_en", GPIOD_OUT_LOW);
	gpiod_set_value(ctx->v18_en, 0);
	devm_gpiod_put(ctx->dev, ctx->v18_en);

	ctx->v33_en = devm_gpiod_get(ctx->dev, "v33_en", GPIOD_OUT_LOW);
	gpiod_set_value(ctx->v33_en, 0);
	devm_gpiod_put(ctx->dev, ctx->v33_en);	

	ctx->v12_en = devm_gpiod_get(ctx->dev, "v12_en", GPIOD_OUT_LOW);
	gpiod_set_value(ctx->v12_en, 0);
	devm_gpiod_put(ctx->dev, ctx->v12_en);
	

	
	printk("set fpga pin ok111\n");
//end	 
...
static const struct drm_display_mode default_mode = {
	.clock = 36456,
	.hdisplay = 800,
	.hsync_start = 800 + 76,//HFP
	.hsync_end = 800 + 76 + 12,//HSA
	.htotal = 800 + 76 + 12 + 60,//HBP
	.vdisplay = 600,
	.vsync_start = 600 + 54,//VFP
	.vsync_end = 600 + 54 + 10,//VSA
	.vtotal = 600 + 54 + 10 + 10,//VBP
};
#endif

#if defined(CONFIG_MTK_PANEL_EXT)
static struct mtk_panel_params ext_params = {
	//.pll_clk = 450,		//595  396   not use
	//.pll_clk = 595,
	//.vfp_low_power = 840,
	//.cust_esd_check = 1,
	//.esd_check_enable = 1,
...
#if 0	
	ctx->v12_en =
		devm_gpiod_get(ctx->dev, "v12_en", GPIOD_OUT_HIGH);
	gpiod_set_value(ctx->v12_en, on);
	devm_gpiod_put(ctx->dev, ctx->v12_en);
	
	ctx->v18_en =
		devm_gpiod_get(ctx->dev, "v18_en", GPIOD_OUT_HIGH);
	gpiod_set_value(ctx->v18_en, on);
	devm_gpiod_put(ctx->dev, ctx->v18_en);
	
	ctx->v33_en =
		devm_gpiod_get(ctx->dev, "v33_en", GPIOD_OUT_HIGH);
	gpiod_set_value(ctx->v33_en, on);
	devm_gpiod_put(ctx->dev, ctx->v33_en);
#endif
//end	
...
//msx-add
	...
	if (IS_ERR(ctx->v12_en)) {
		dev_info(dev, "moshaoxi cannot get v12_en-gpios %ld\n",
			 PTR_ERR(ctx->v12_en));
	...
	if (IS_ERR(ctx->v18_en)) {
		dev_info(dev, "moshaoxi cannot get v18_en-gpios %ld\n",
			 PTR_ERR(ctx->v18_en));
	...
	if (IS_ERR(ctx->v33_en)) {
		dev_info(dev, "moshaoxi cannot get v33_en-gpios %ld\n",
			 PTR_ERR(ctx->v33_en));

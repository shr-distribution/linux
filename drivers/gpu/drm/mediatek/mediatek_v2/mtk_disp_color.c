	...
	unsigned int *p_split_window_x, unsigned int *p_split_window_y)
{
	...
	if (comp->mtk_crtc->is_dual_pipe) {
		...
		printk("g_color0_dst_w/h not init, return default settings\n");
		printk("moshaoxi222,log\n");
	} else if (g_split_en) {
		printk("moshaoxi3333,log\n");
		/* TODO: CONFIG_MTK_LCM_PHYSICAL_ROTATION other case */
		if (params && params->rotate == MTK_PANEL_ROTATE_180) {
			...
			//kcm added by phf, 20240813
			if (params && params->rotate == MTK_PANEL_ROTATE_270)
			{
				printk("moshaoxi444,log\n");
				split_window_x = ((g_split_window_y_end) << 16) |
						g_split_window_y_start;
				split_window_y =
				 ((g_color_dst_h[id] - g_split_window_x_start) << 16) |
				 (g_color_dst_h[id] - g_split_window_x_end);
			}
			else
			//kcm added end	
			{

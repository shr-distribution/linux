...
static void mtk_wdma_start(struct mtk_ddp_comp *comp, struct cmdq_pkt *handle)
{
	...
	if (wdma->info_data && wdma->info_data->use_larb_control_sec && crtc_idx == 2) {
			      ...
			      uint32_t sec, void *handle)
{
	...
	if (wdma->data) {
		if (wdma->info_data && wdma->info_data->use_larb_control_sec) {
...
static int mtk_disp_wdma_probe(struct platform_device *pdev)
{
	...
	priv->info_data->use_larb_control_sec = priv->data->use_larb_control_sec;
	if (priv->data->use_larb_control_sec && !mtk_disp_is_svp_on_mtee()) {
		priv->info_data->use_larb_control_sec = false;
	}

				...
				struct dma_async_tx_descriptor **dma_desc)
{
	...
	if (host->variant->qcom_dml && host->mmc->index == 1) {
		 ...
		 * validate_dma() so 128 B HTC mailbox traffic goes via DMA on
		 * ADM channel 21 with 64-byte rows.
		 *
		 * The 2026-06-11 256 B threshold workaround (force PIO < 256
		 * to dodge mailbox DMA lookahead corruption at the then-active
		 * 24 MHz MCLK) is retired together with the ath6kl_sdio_io
		 * mailbox-read PIO split.  With sdcc4 at 48 MHz (bc9052005979),
		 * mmci defensive stale-clear (e6bfb8e97687), ath6kl SI retry
		 * for register-space CMD53s (db48f4b87bf9), and the post-BMI
		 * settle delay (cfdae4653755), the qcom_dml DMA path is
		 * byte-correct for all mmc1 mailbox traffic.
		 */

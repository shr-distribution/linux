...
static irqreturn_t mtk_rtc_irq_handler_thread(int irq, void *data)
{
	...
	if (pwron_alarm) {
		...
		if (now_time >= time - 1 && now_time <= time + 4) {
			...
			dev_warn(rtc->rtc_dev->dev.parent, "WARNING: poffalm may miss!\n");
			dev_warn(rtc->rtc_dev->dev.parent, "now_time:%04d/%02d/%02d %02d:%02d:%02d time(spare):%04d/%02d/%02d %02d:%02d:%02d\n",
			...
			if (bootmode == KERNEL_POWER_OFF_CHARGING_BOOT ||
				...
				if (now_time > time + 90) {
					dev_err(rtc->rtc_dev->dev.parent, "%s now_time > time + 90", __func__);
				}
				mtk_rtc_update_pwron_kpoc_flag(rtc);
				regmap_read(rtc->regmap, rtc->addr_base + RTC_PDN1, &__pdn1);
				dev_notice(rtc->rtc_dev->dev.parent, "pdn1 = 0x%x\n", __pdn1);

 ...
 *
 * backported based on: http://www.mail-archive.com/linux-omap@vger.kernel.org/msg03590.html
 * integrated patch: http://www.mail-archive.com/linux-omap@vger.kernel.org/msg06960.html
 * plus some patches from OpenPandora driver
 *
...
#define REG_BB_CFG             0x012	// 500 uA
#define BBCHEN                 0x010
...
static inline int twl4030charger_presence_evt(void)
{
       ...
       if (ret)
               return IRQ_NONE;
	printk("twl4030charger_presence_evt() REG_STS_HW_CONDITIONS %02x\n", chg_sts);
...
static int twl4030charger_presence(void)
{
       ...
       ret = twl_i2c_read_u8(TWL4030_MODULE_PM_MASTER, &hwsts,
               REG_STS_HW_CONDITIONS);
 	printk("twl4030charger_presence() REG_STS_HW_CONDITIONS %02x\n", hwsts);
      if (ret) {
...
//	printk("twl4030_bk_bci_battery_work\n");
...
// 	printk("twl4030_bci_battery_work\n");
...
static void twl4030_bci_battery_external_power_changed(struct power_supply 
*psy)
{
...
batt_failed:
	irq = platform_get_irq(dev, 0);
	free_irq(irq, NULL);chg_irq_fail:
prev_setup_err:
	irq = platform_get_irq(dev, 1);
	free_irq(irq, NULL);batt_irq_fail:
voltage_setup_fail:
temp_setup_fail:

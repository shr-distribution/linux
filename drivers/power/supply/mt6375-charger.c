...
#ifdef CONFIG_CPS_WLS_CHARGER
#include "cps_wls_charger.h"
static int wls_max_charging_current = 0;
#endif
				   ...
				   const union power_supply_propval *val)
{
...
#ifdef CONFIG_CPS_WLS_CHARGER
		if(wls_mod0_gp1_get()){
			mt6375_get_vbus(ddata->chgdev, &vbus);
			mt_dbg(ddata->dev, "mt6375_get_vbus=%d\n", vbus);
			if(vbus > 7000000){
				vbus_flag = 1;
			}else{
				vbus_flag = 0;
			...
			if(vbus_flag){
				if(val->intval >= 2200){
					wls_max_charging_current = 2200;
				}else{
					wls_max_charging_current = val->intval;
				}
			}else{
				if(val->intval >= 800){
					wls_max_charging_current = 800;
				}else{
					wls_max_charging_current = val->intval;
				}
			}
			if(wls_max_charging_current >= 500){
				for(i=0 ; i <= (wls_max_charging_current / 100) -5 ; i++)//(val->intval / 100) -4
				{
					if(cps_wls_current < wls_max_charging_current )
						cps_wls_current = i * 100 + 500;
					mt_dbg(ddata->dev, "set charging current=%d\n", cps_wls_current);
					mutex_lock(&ddata->pe_lock);	
					ret = mt6375_chg_field_set(ddata, F_CC, cps_wls_current );
					mutex_unlock(&ddata->pe_lock);
					if(cps_wls_current >= wls_max_charging_current )
						break;
					msleep(400);
				...
				mutex_lock(&ddata->pe_lock);	
				ret = mt6375_chg_field_set(ddata, F_CC, wls_max_charging_current);
				mutex_unlock(&ddata->pe_lock);
			}
		}else{
			cps_wls_current = 0;
	...
	case POWER_SUPPLY_PROP_INPUT_CURRENT_LIMIT:
		...
		if(wls_mod0_gp1_get()){
			mt6375_get_vbus(ddata->chgdev, &vbus);
			mt_dbg(ddata->dev, "POWER_SUPPLY_PROP_INPUT_CURRENT_LIMIT mt6375_get_vbus=%d\n", vbus);
			if(vbus > 7000000){
				vbus_flag = 1;
			}else{
				vbus_flag = 0;
			...
			if(vbus_flag){
				mt_dbg(ddata->dev, "set aicl 1800\n");
				ret = mt6375_chg_field_set(ddata, F_IAICR, 1000);
			}else{
				mt_dbg(ddata->dev, "set aicl 500\n");
				ret = mt6375_chg_field_set(ddata, F_IAICR, 600);
			}
				
		}else{
#endif
			mutex_lock(&ddata->pe_lock);
			ret = mt6375_chg_field_set(ddata, F_IAICR, 3200);
			mutex_unlock(&ddata->pe_lock);
#ifdef CONFIG_CPS_WLS_CHARGER
		}
#endif
...
static int mt6375_enable_charging(struct charger_device *chgdev, bool en)
{
	...
	/* boot_mode 1:META_BOOT 5:ADVMETA_BOOT moshaoxi */   
	// boot_mode 1:META_BOOT 5:ADVMETA_BOOT moshaoxi
	if (pdata->boot_mode == 1 || pdata->boot_mode == 5) {
		printk("boot_mode to disable charging\n");
		mt6375_chg_field_set(ddata, F_HZ, 1);
		return mt6375_chg_enable_charging(ddata, 0);
	}

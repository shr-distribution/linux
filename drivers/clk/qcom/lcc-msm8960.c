...
static int lcc_msm8960_probe(struct platform_device *pdev)
{
	...
	if (of_device_is_compatible(pdev->dev.of_node, "qcom,lcc-msm8260") ||
		 ...
		 * window when the clock is being disabled. The OSR clock's
		 * HALT (bit 1) does assert, so this is not a power-up or
		 * clock-routing issue; only the bit_div branch's poll is
		 * unreliable on this hardware. Skip the check so PCM stop

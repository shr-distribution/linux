 ...
 */
/*
 * Vote @rate (Hz) for a fabric / EBI / SMI bus-clock RPM resource and
 * wait for the firmware to apply at least that rate to the physical
 * fabric before returning.
 *
 * Uses qcom_rpm_write_sync() for the ACTIVE_STATE write so a consumer
 * that calls icc_set_bw() and then immediately issues a DMA is
 * guaranteed the fabric ramp has completed (status register has
 * converged) before its DMA hits the wire.  This is the mechanism the
 * v5 design block in msm8660_icc_set() referred to as "until a
 * `qcom_rpm_write_sync` (status-register polling) helper exists in
 * drivers/mfd/qcom_rpm.c"; that helper is now in tree, so dynamic
 * scaling is safe.
 *
 * SLEEP_STATE writes do not poll: the per-resource status register
 * only reflects sleep-context values during cluster sleep, so polling
 * during the active phase would just time out.  The plain
 * qcom_rpm_write() ack is the strongest synchronisation available
 * for sleep votes.
 */

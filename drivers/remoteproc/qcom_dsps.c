...
/*
 * Qualcomm MSM8x60 family (MSM8260/MSM8660/APQ8060) DSPS (Dedicated Sensors Processor Subsystem)
...
static int qcom_dsps_probe(struct platform_device *pdev)
{
	...
	if (dsps->use_pas) {
		...
		if (IS_ERR(dsps->pas_ctx))
			return dev_err_probe(dev, PTR_ERR(dsps->pas_ctx),
					     "Failed to alloc PAS context\n");
	}

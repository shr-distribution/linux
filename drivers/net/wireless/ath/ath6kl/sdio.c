			  ...
			  u8 *buf, u32 len)
{
	...
	if (request & HIF_WRITE) {
		...
		/*
		 * Historical 2026-06-11 workaround (now removed): mailbox reads
		 * of >= 64 B were split into 32 B PIO chunks because 128 B DMA
		 * reads on the qcom_adm SDIO path returned corrupted lookahead
		 * data.  The corruption was provoked by the 24 MHz MCLK then
		 * in effect on this hardware.  After bumping sdcc4 MCLK to
		 * 48 MHz (bc9052005979), adding the mmci defensive stale-clear
		 * (e6bfb8e97687), and adding the SI retry at
		 * ath6kl_sdio_read_write_sync (db48f4b87bf9), the underlying
		 * qcom_adm + mmci-pl18x mailbox-read DMA path is clean.  Drop
		 * the split: each HTC RX is now a single DMA transaction, which
		 * recovers the ~3x throughput legacy webOS msm_sdcc.c achieves
		 * with its ADM channel 21 DMA descriptor (src_dst_len=0x400040,
		 * row_offset=0x40, 4 rows x 64 B).
		 *
		 * If the lookahead corruption returns on this code path, the
		 * fastest diagnostic is `ath6kl_dbg(ATH6KL_DBG_HTC, ...)` plus
		 * `dmesg | grep "lk_ahd mismatch"`.
		 */
		if (request & HIF_FIXED_ADDRESS)

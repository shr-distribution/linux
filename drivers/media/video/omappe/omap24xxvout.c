...
init_vout_device (int vid , int dev_index)
{
	int r, k;
	struct omap24xxvout_device *vout;
	struct video_device *vfd;
	...
	if (video_register_device (vfd, VFL_TYPE_GRABBER, dev_index) < 0){
...
omap24xxvout_probe (struct platform_device *pdev)
{
	...
	if (pdev->dev.platform_data) {
		...
		if (vout_mem_plat->use_forced_index) {
			forced_index = vout_mem_plat->forced_index;
		}
	}

	if (pdev->id == 11) {	// video1out
		saved_v1out = init_vout_device(OMAP2_VIDEO1, ((forced_index>=0)?forced_index:OMAP2_VIDEO1));
		if (saved_v1out == NULL) {
			omap2_disp_put_dss();
			return -ENODEV;
		}
		...
		saved_v2out = init_vout_device(OMAP2_VIDEO2, ((forced_index>=0)?forced_index:OMAP2_VIDEO1));
		if (saved_v2out == NULL) {
			omap2_disp_put_dss();
			return -ENODEV;
		}

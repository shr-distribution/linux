...
static int mt9m113_sensor_init(struct mt9m114 *sensor)
{
	...
	/* Debug: Read back a 0x30xx register to see if init table writes work */
	{
		u64 readback = 0;
		/* 0x3044 was written with 0x0504 in the init table */
		cci_read(sensor->regmap, CCI_REG16(0x3044), &readback, NULL);
		dev_info(dev, "MT9M113: reg 0x3044 readback=0x%llx (expect 0x0504)\n",
			 readback);
		/* Also check 0x301A after table */
		cci_read(sensor->regmap, MT9M114_RESET_REGISTER, &readback, NULL);
		dev_info(dev, "MT9M113: RESET_REGISTER=0x%llx (after init table)\n",
			 readback);
	}

	/*
	 * Debug: Try raw I2C read/write to 0x301A to verify CCI vs raw I2C.
	 * This bypasses the CCI/regmap layer entirely.
	 */
	{
		...
		/* Raw I2C read of 0x301A */
		wbuf[0] = 0x30;  /* Addr high byte */
		wbuf[1] = 0x1A;  /* Addr low byte */
		...
		if (rc == 2) {
			raw_val = (rbuf[0] << 8) | rbuf[1];
			dev_info(dev, "MT9M113: RAW I2C read 0x301A = 0x%04x\n", raw_val);
		} else {
			dev_err(dev, "MT9M113: RAW I2C read 0x301A failed: %d\n", rc);
		}

		/* Raw I2C write 0x120C to 0x301A */
		wbuf[0] = 0x30;  /* Addr high */
		wbuf[1] = 0x1A;  /* Addr low */
		wbuf[2] = 0x12;  /* Data high */
		wbuf[3] = 0x0C;  /* Data low */
		msgs[0].addr = client->addr;
		msgs[0].flags = 0;
		msgs[0].len = 4;
		msgs[0].buf = wbuf;
		rc = i2c_transfer(client->adapter, msgs, 1);
		dev_info(dev, "MT9M113: RAW I2C write 0x301A=0x120C rc=%d\n", rc);
		...
		if (rc == 2) {
			raw_val = (rbuf[0] << 8) | rbuf[1];
			dev_info(dev, "MT9M113: RAW I2C read 0x301A after write = 0x%04x (expect 0x120C)\n",
				 raw_val);
		}
	}

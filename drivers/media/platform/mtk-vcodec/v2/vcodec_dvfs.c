...
u64 calc_freq(struct vcodec_inst *inst, struct mtk_vcodec_dev *dev)
{
	...
	if (inst->codec_type == MTK_INST_DECODER) {
		...
		(inst->width * inst->height <= 1280 * 736)) {
			...
			if (inst->priority < 0) {
				...
				if (inst->codec_fmt == 808996950) {
					...
					inst->op_rate = 60;
				} else if (inst->codec_fmt == 809062486 || inst->codec_fmt == 892744264) {
					/* performance class WA for VP9 and HEVC*/
					inst->op_rate = 120;
		...
		if (perf != 0) {
			...
			freq = div_u64(freq, inst->core_cnt);
			/* SW overhead */
			if (inst->width * inst->height <= 1920 * 1088)
				freq = div_u64(freq, 10) * 11;
			else if (inst->width * inst->height <= 1280 * 736)
				freq = div_u64(freq, 10) * 12;

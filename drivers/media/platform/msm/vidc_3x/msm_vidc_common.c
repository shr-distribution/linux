...
int msm_comm_qbuf(struct msm_vidc_inst *inst, struct vb2_buffer *vb)
{
	 ...
	 */
	if(!defer) defer = inst->state != MSM_VIDC_START_DONE;
	 ...
	 */
	if(!defer) defer = vbuf && vbuf->flags & V4L2_MSM_BUF_FLAG_DEFER;

	/* 3) If we're in batch mode, we must have full batches of both types */
	if(!defer) defer = batch_mode && (!output_count || !capture_count);

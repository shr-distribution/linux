...
#define VIDC_PIX_CACHE_TAG_CLEAR_BIT	BIT(0)	/* REG_22756[0] */
#define VIDC_PIX_CACHE_SW_RESET_BIT	BIT(1)	/* REG_169013[1] */
#define VIDC_PIX_CACHE_TILE_FACTOR	8192	/* VIDC_TILE_MULTIPLY_FACTOR */
#define VIDC_PIX_CACHE_MAX_DPB		19
#define VIDC_PIX_CACHE_DPB_RESET	0xfffffff8 /* VIDC_1080P_DEC_DPB_RESET_VALUE */
...
struct vidc_core {
	...
	/*
	 * VCODEC core + AXI resets. The VED GDSC only cycles VCODEC_AHB_RESET
	 * on power-on, which leaves the RISC/core warm across a power-collapse;
	 * pulsing these (clocks on) in runtime_resume gives a cold core, the
	 * way the legacy footswitch did. Optional: NULL if the DT omits resets.
	 */
	struct reset_control *rst_core;
	struct reset_control *rst_axi;
	struct reset_control *rst_axi_a;
	struct reset_control *rst_axi_b;

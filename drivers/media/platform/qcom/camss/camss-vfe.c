...
static void vfe31_debug_dump_clock_state(struct device *dev)
{
	...
	/* Check CSI mux selection status (actual configuration done in vfe_get) */
	if (misc_cc_reg != 0x06003400) {
		dev_info(dev, "  Note: MISC_CC will be set to 0x06003400 in vfe_get()\n");
	}
...
static void vfe31_force_enable_axi_clock(struct device *dev)
{
	...
	vfe_cc_reg = readl_relaxed(mmcc_base + MSM8660_VFE_CC_REG_OFFSET);
	dev_info(dev, "VFE AXI: VFE_CC_REG before = 0x%08x\n", vfe_cc_reg);

	if (!(vfe_cc_reg & MSM8660_VFE_CC_REG_VFE_AXI_EN)) {
		dev_warn(dev, "VFE AXI: VFE_AXI_CLK (bit 1) NOT enabled, forcing on\n");
		vfe_cc_reg |= MSM8660_VFE_CC_REG_VFE_AXI_EN;
		writel_relaxed(vfe_cc_reg, mmcc_base + MSM8660_VFE_CC_REG_OFFSET);
		wmb();
		vfe_cc_reg = readl_relaxed(mmcc_base + MSM8660_VFE_CC_REG_OFFSET);
		dev_info(dev, "VFE AXI: VFE_CC_REG after = 0x%08x\n", vfe_cc_reg);
	}
			     ...
			     u16 width, u16 height)
{
	...
	if (enable) {
		 ...
		 */
		{
			...
			writel_relaxed(0x40, vfe->base + 0x1E4);

			/* SUBSAMPLE_CFG_0 (0x1F4) = height - 1 per webOS */
			writel_relaxed(height - 1, vfe->base + 0x1F4);
			/* SUBSAMPLE_CFG_1 (0x1F8) = 0xFFFFFFFF per webOS */
			writel_relaxed(0xFFFFFFFF, vfe->base + 0x1F8);
...
void vfe_enable_pending_camif(struct vfe_device *vfe)
{
	 ...
	 * - AXI output mode at 0x040 (0x60 for raw, 0x01 for preview+video)
	 ...
	 */
	dev_info(vfe->camss->dev, "VFE: EFS_CFG=0x40 at 0x1E4 (webOS value)\n");
	 ...
	 */
	{
		u32 subsample_height = line->fmt[MSM_VFE_PAD_SINK].height;
		if (vfe31_window_height_override > 0)
			subsample_height = vfe31_window_height_override;
		dev_info(vfe->camss->dev, "VFE: SUBSAMPLE_CFG_0=0x%08x (height-1)\n",
			 subsample_height - 1);
		writel_relaxed(subsample_height - 1, vfe->base + VFE31_CAMIF_SUBSAMPLE_CFG_0);
	}
	/*
	 * SUBSAMPLE_CFG_1 at 0x1F8: webOS uses 0xFFFFFFFF (no frame skip)
	 */
	dev_info(vfe->camss->dev, "VFE: SUBSAMPLE_CFG_1=0xFFFFFFFF (no skip)\n");
...
#define VFE31_BUS_CMD_RELOAD_WM		0x3FFF	/* webOS value */
	 ...
	 *
	 * webOS register dump shows value 0x00220011 during preview mode:

...
struct key_data {
	...
	int num_adjacent;
	bool up;
	struct timespec64 down_time;
	struct timespec64 up_time;
	int count;
	bool send_count;
	bool inadv_check;
	bool modifier;
...
struct stmpe_keypad {
	...
	int log_idx;
	bool reset_detect;
	struct timespec64 last_keypress;
				...
				key_event_type_t type)
{
	...
	if (type == KEY_EVENT_DOWN) {
		...
		ktime_get_real_ts64(&keypad->keys[key].up_time);
		key_duration = ((keypad->keys[key].up_time.tv_sec -
				keypad->keys[key].down_time.tv_sec) * 1000) +
			((keypad->keys[key].up_time.tv_nsec -
				keypad->keys[key].down_time.tv_nsec) / 1000000);
	...
	for (n = 0; n < KEYPAD_EVENT_LOG_SIZE; n++) {
		...
		if (keypad->event_log[i].key != 0)
			size += snprintf(buf + size, PAGE_SIZE - size,
				...
				keypad->event_log[i].time.tv_nsec);
	}
			...
			uint8_t addr, uint8_t length, uint8_t *data)
{
	...
	msg[0].addr = keypad->i2c_client->addr;
	msg[0].flags = 0;
	msg[0].len = length + 1;
	msg[0].buf = buf;
...
int stmpe_handle_keypress(struct stmpe_keypad *keypad)
{
	...
	do {
		...
		for (i = 0; i < num_events; i++) {
			...
			ktime_get_real_ts64(&keypad->last_keypress);
...
static void stmpe_keypad_inadv_handler(struct timer_list *t)
{
	struct stmpe_keypad *keypad = from_timer(keypad, t, inadv_timer);
	uint8_t key;
...
static void stmpe_keypad_timer_handler(struct timer_list *t)
{
	struct stmpe_keypad *keypad = from_timer(keypad, t, timer);
		...
		const struct i2c_device_id *dev_id)
{
	...
	timer_setup(&keypad->timer, stmpe_keypad_timer_handler, 0);
	timer_setup(&keypad->inadv_timer, stmpe_keypad_inadv_handler, 0);

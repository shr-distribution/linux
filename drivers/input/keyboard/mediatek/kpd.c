...
#ifdef CONFIG_PM_SLEEP
	if (pressed)
		__pm_stay_awake(kpd_suspend_lock);
	else
		__pm_relax(kpd_suspend_lock);
#endif
}
#endif
...
static void kpd_keymap_handler(unsigned long data)
{
	...
	if(kpd_suspend_flag == 1)
	{
        input_report_key(kpd_input_dev, KEY_1, 1);
		input_sync(kpd_input_dev);
		input_report_key(kpd_input_dev, KEY_1, 0);
		input_sync(kpd_input_dev);
		//kpd_suspend_flag = 0;
	}
	else
		kpd_print("yqf kpd_keymap_handler resume \n");
	...
	kpd_print("yqf report Linux keycode = %d\n", linux_keycode);
...
static int kpd_pdrv_probe(struct platform_device *pdev)
{
	...
	if (!kpd_suspend_lock) {
		kpd_notice("register wakeup source failed\n");

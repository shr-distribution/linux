...
#define GSL_LOGD(fmt, args...)                                                 \
	printk(GSLX680_NAME "<-dbg-> [%04d] [@%s]" fmt, __LINE__, __func__,  \
		 ##args)
#define GSL_LOGF()                                                             \
	printk(GSLX680_NAME "<-dbg-> [%04d] [@%s] is call!\n", __LINE__,    \
		 __func__)
...
static int power_check(struct i2c_client *client)
{
	...
	GSL_LOGD("#########gsl---check mem read 0xb0 = %x %x %x %x #########\n", read_buf[3], read_buf[2], read_buf[1], read_buf[0]);
	if (read_buf[3] != 0x5a || read_buf[2] != 0x5a
			|| read_buf[1] != 0x5a || read_buf[0] != 0x5a)
	{

	    GSL_LOGD("#########gsl---ccheck mem read 0xb0 = %x %x %x %x #########\n", read_buf[3], read_buf[2], read_buf[1], read_buf[0]);
...
static ssize_t gsl_sysfs_tpgesturet_store(struct device *dev,struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if(buf[0] == '0'){
	//	gsl_gesture_flag = 0;
		GSL_LOGD("[GSL_GESTURE] gsl_sysfs_tpgesturet_store off.\n");
	}else if(buf[0] == '1'){
		gsl_gesture_flag = 1;
		GSL_LOGD("[GSL_GESTURE] gsl_sysfs_tpgesturet_store on.\n");
	}
...
static unsigned int gsl_gesture_init(void)
{
	...
	{
		GSL_LOGD("%s: subsystem_register failed\n", __func__);
		return -ENOMEM;
	}
    ...
    if (ret)
    {
        GSL_LOGD("%s: sysfs_create_version_file failed\n", __func__);
        return ret;
    }

	gsl_request_power_idev();

	GSL_LOGD("[GSL_GESTURE] gsl_gesture_init success.\n");
	return 1;
}
...
#ifdef GSL_GESTURE
		GSL_LOGD("[GSL_GESTURE] tpd_halt =%d, gsl_gesture_flag = %d \n",tpd_halt, gsl_gesture_flag);
		if(gsl_gesture_flag == 1 && 0 == gsl_lcd_flag){
			int tmp_c;
			tmp_c = gsl_obtain_gesture();
			GSL_LOGD("[GSL_GESTURE] tmp_c =%d \n",tmp_c);
			if(tpd_halt == 1) 
			{
				switch(tmp_c){
					//case (int)'*':
					//	gsl_gesture_c = (char)tmp_c;
						//GSL_LOGD("gesture,gsl_lcd_flag=%d\n",gsl_lcd_flag); 	
					//	break;
					case 42:
						gsl_gesture_c = (char)tmp_c;
						//GSL_LOGD("gesture,gsl_lcd_flag=%d\n",gsl_lcd_flag); 	
					...
					default:
						...
						//gsl_lcd_flag = 1;
						//tpd_halt = 0; 
						GSL_LOGD("[GSL_GESTURE] >>> default == tmp_c \n"); 							
						break;
				}
...
static void gsl_monitor_worker(struct work_struct *work)
{
	...
	if(init_chip_flag){
		...
		// ret = regulator_set_voltage(tpd->reg, 0, 0);
		// if (ret)
			 ...
			 const struct i2c_device_id *id)
{
	...
	if (node) {
		...
		if (enable_irq_wake(touch_irq) < 0)
		    GSL_LOGE("irq %d enable irq wake fail\n", touch_irq);
	} else {
		ret = -1;
...
/*			input_set_capability(tpd->dev, EV_KEY, KEY_C);
			...
			input_set_capability(tpd->dev, EV_KEY, KEY_I);
...
static void tpd_suspend(struct device *h)
{
	...
	i2c_lock_flag=0;
	 ...
	 if(gsl_gesture_flag == 1){
		 ...
		 i2c_smbus_write_i2c_block_data(i2c_client,0x8,4,buf);
		 msleep(50);
		 GSL_LOGD(">>>> gsl_gesture_flag == 1 return! \n");
		 return;
	 }
#endif
...
static void tpd_resume(struct device *h)
{
	...
	if(gsl_gesture_flag == 1)
	{
		...
		//unsigned int temp;
		gsl_lcd_flag = 1;
		//ret = disable_irq_wake(touch_irq);
		tpd_gpio_output(GTP_RST_PORT, 0);
		msleep(20);
		tpd_gpio_output(GTP_RST_PORT, 1);
		msleep(20);

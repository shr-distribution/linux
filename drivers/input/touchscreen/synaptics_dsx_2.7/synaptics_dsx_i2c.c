...
*/
extern struct device *g_rmi4_dev;
static ssize_t wakeup_gesture_write(struct file *file, const char __user *buffer,
...
static ssize_t wakeup_gesture_read(struct file *file, char __user *page, size_t count, loff_t *ppos)
{
	struct synaptics_rmi4_data *rmi4_data = dev_get_drvdata(g_rmi4_dev);
	char *ptr = page;
		...
		unsigned short addr, unsigned char *data, unsigned int length)
{
	...
	msg = kmalloc_array(rd_msgs + 1, sizeof(struct i2c_msg), GFP_KERNEL);
	if (!msg)
		return -ENOMEM;
...
struct proc_dir_entry *synaptics_config_proc = NULL;
		...
		const struct i2c_device_id *dev_id)
{
	...
	synaptics_config_proc = proc_create("wakeup_gesture", 0777, NULL, &gesture_proc_fops);
	if(!synaptics_config_proc)
	{
		dev_err(&client->dev,
...
static int synaptics_rmi4_i2c_remove(struct i2c_client *client)
{
	...
	if(NULL!=synaptics_config_proc)
	{
		proc_remove(synaptics_config_proc);
	}

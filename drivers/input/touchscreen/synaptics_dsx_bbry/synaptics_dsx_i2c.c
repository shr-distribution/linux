				...
				struct synaptics_dsx_board_data *bdata)
{
	...
	if (of_find_property(np, "synaptics,power_2p8_en", NULL)) {
		bdata->power_2p8_en = of_get_named_gpio_flags(np,
				"synaptics,power_2p8_en", 0, NULL);
		if (!gpio_is_valid(bdata->power_2p8_en)) {
			dev_err(dev, "%s: power_2p8_en GPIO %d, is invalid\n",
				__func__, bdata->power_2p8_en);
			return -EPROBE_DEFER;
		}
	} else {
		bdata->power_2p8_en = -1;
	}

	if (of_find_property(np, "synaptics,power_1p8_en", NULL)) {
		bdata->power_1p8_en = of_get_named_gpio_flags(np,
				"synaptics,power_1p8_en", 0, NULL);
		if (!gpio_is_valid(bdata->power_1p8_en)) {
			dev_err(dev, "%s: power_1p8_en GPIO %d, is invalid\n",
				__func__, bdata->power_1p8_en);
			return -EPROBE_DEFER;
		}
	} else {
		bdata->power_1p8_en = -1;
	}
	...
	if (bdata->reg_en) {
		...
		bdata->vcc_2p8v = regulator_get(dev, "vcc_2p8v");
		if (IS_ERR(bdata->vcc_2p8v)) {
			retval = PTR_ERR(bdata->vcc_2p8v);
			dev_err(dev,
				"Regulator get failed vcc_2p8v rc=%d\n", retval);
			bdata->reg_en = false;
		}
	}
	...
	if (of_property_read_u32(np, "synaptics,panel_x",
			&bdata->panel_x))
		bdata->panel_x = 0;
	if (of_property_read_u32(np, "synaptics,panel_y",
			&bdata->panel_y))
		bdata->panel_y = 0;
	if (of_property_read_u32(np, "synaptics,num_of_rx_electrodes",
			&bdata->num_of_rx_electrodes))
		bdata->num_of_rx_electrodes = 0;
		...
		const struct i2c_device_id *dev_id)
{
	...
	if (IS_ERR(default_pctrl)) {
		dev_err(&client->dev,
			"%s: Please add default pinctrl state\n",
			__func__);
...
int synaptics_bbry_rmi4_bus_init(void)
{
	return i2c_add_driver(&synaptics_rmi4_i2c_driver);
}
EXPORT_SYMBOL(synaptics_bbry_rmi4_bus_init);

void synaptics_bbry_rmi4_bus_exit(void)
{
	i2c_del_driver(&synaptics_rmi4_i2c_driver);

	return;
}
EXPORT_SYMBOL(synaptics_bbry_rmi4_bus_exit);

...
#include "mtk_disp_notify.h"            //msx-add 241118
...
static int tp_drm_notifier_callback(struct notifier_block *nb,
		unsigned long value, void *v)
{
	...
	printk("%s enter: value:0x%x, *data:0x%x in\n", __func__, value, *data);
	if (ts_data && v) {
		if (value == MTK_DISP_EVENT_BLANK) {
			if (*data == MTK_DISP_BLANK_UNBLANK && suspend_flag) {
				...
				//printk("%s: resume out\n", __func__);
					...
					//printk("%s: suspend out\n", __func__);
			}
		} else if (value == MTK_DISP_EARLY_EVENT_BLANK) {
				if (*data == MTK_DISP_BLANK_POWERDOWN) {
					//printk("%s: ealy suspend in\n", __func__);
					suspend_flag = 1;
					//printk("%s: ealy suspend out\n", __func__);
			}
		} else {
			printk("%s: tp_drm can not suspend or resume\n", __func__);
				return -1;
		}
	}
	return 0;
...
static int fts_ts_probe_entry(struct fts_ts_data *ts_data)
{
    ...
    //msx-add 241118
	ts_data->disp_notifier.notifier_call = tp_drm_notifier_callback;
	if (mtk_disp_notifier_register("ft_tp_drm", &ts_data->disp_notifier))
		printk("Failed to register ft_tp_drm disp notifier client");		
    //end
...
static int fts_ts_suspend(struct device *dev)
{
    ...
    if (ts_data->gesture_support) {
        ...
        if (!ts_data->ic_info.is_incell) {
            ...
            printk("ts_data->ic_info.is_incell enter\n");

...
#include <linux/fb.h>
	...
	#if defined(FOCAL_CONFIG_DRM_PANEL)
...
static struct platform_device *focaltech_pdev;
/*****************************************************************************
...
int fts_wait_tp_to_valid(void)
{
    ...
    do {
        ...
        if ((idh == chip_idh) || (fts_check_cid(ts_data, idh) == 0)) {
            ...
            FTS_INFO("TP Not Ready,ReadData:0x%02x,ret:%d", idh, ret);
...
void fts_hid2std(void)
{
    ...
    ret = fts_write(buf, 3);
    if (ret < 0) {
        FTS_INFO("hid2std cmd write fail");
    } else {
        msleep(10);
        buf[0] = buf[1] = buf[2] = 0;
        ret = fts_read(NULL, 0, buf, 3);
        if (ret < 0) {
            FTS_INFO("hid2std cmd read fail");
                         ...
                         u16 type, u8 id_h, u8 id_l, bool force)
{
    ...
    for (j = 0; j < FTS_MAX_CHIP_IDS; j++) {
        if (id == chip_id_list[i].chip_ids[j]) {
            FTS_INFO("cid:%x==%x", id, chip_id_list[i].chip_ids[j]);
            FTS_INFO("match cid,type:0x%x", (int)chip_id_list[i].type);
            ts_data->ic_info.cid = chip_id_list[i];
            return 0;
        }
...
static int fts_read_bootid(struct fts_ts_data *ts_data, u8 *id)
{
    ...
    if ((ret < 0) || (0x0 == chip_id[0]) || (0x0 == chip_id[1])) {
        FTS_INFO("read boot id fail,read:0x%02x%02x", chip_id[0], chip_id[1]);
        return -EIO;
    }
...
static int fts_get_ic_information(struct fts_ts_data *ts_data)
{
    ...
    if ((cnt * INTERVAL_READ_REG) >= TIMEOUT_READ_REG) {
        ...
        if (ret <  0) {
            FTS_INFO("read boot id fail");
            return ret;
        }

        ret = fts_get_chip_types(ts_data, chip_id[0], chip_id[1], INVALID);
        if (ret < 0) {
            FTS_INFO("can't get ic informaton");
            return ret;
        }
    }
...
static int fts_input_report_a(struct fts_ts_data *ts_data, struct ts_event *events)
{
    ...
    for (i = 0; i < ts_data->touch_event_num; i++) {
        ...
        if (EVENT_DOWN(events[i].flag)) {
            ...
            if ((ts_data->log_level >= 2) ||
                ((1 == ts_data->log_level) && (FTS_TOUCH_DOWN == events[i].flag))) {
                FTS_INFO("[A]P%d(%d, %d)[p:%d,tm:%d] DOWN!",
                          events[i].id, events[i].x, events[i].y,
                          events[i].p, events[i].area);
            }
        }
...
static int fts_input_pen_report(struct fts_ts_data *ts_data, u8 *pen_buf)
{
    ...
    case STYLUS_DEFAULT:
        if (pevt->tip && pevt->p) {
            if ((ts_data->log_level >= 2) || (!pevt->down))
                FTS_INFO("[PEN]x:%d,y:%d,p:%d,tip:%d,flag:%d,tilt:%d,%d DOWN",
            ...
            FTS_INFO("[PEN]x:%d,y:%d,p:%d,tip:%d,flag:%d,tilt:%d,%d UP",
                      pevt->x, pevt->y, pevt->p, pevt->tip, pevt->flag,
                      pevt->tilt_x, pevt->tilt_y);
            input_report_abs(pen_dev, ABS_X, pevt->x);
            input_report_abs(pen_dev, ABS_Y, pevt->y);
    ...
    case STYLUS_HOVER:
        if (ts_data->log_level >= 1)
            FTS_INFO("[PEN][%02X]x:%d,y:%d,p:%d,tip:%d,flag:%d,tilt:%d,%d,%d",
...
static int fts_read_touchdata(struct fts_ts_data *ts_data, u8 *buf)
{
    ...
    if (ret < 0) {
        FTS_INFO("read touchdata fails,ret:%d", ret);
        return ret;
    }
    ...
    if (touch_max_size > ts_data->touch_size) {
        ...
        ret = fts_read(&ts_data->touch_addr, 1, buf + ts_data->touch_size, \
                       touch_max_size - ts_data->touch_size);
        if (ret < 0) {
            FTS_INFO("read touchdata2 fails,ret:%d", ret);
            return ret;
        }
    }
...
static int fts_read_parse_touchdata(struct fts_ts_data *ts_data, u8 *touch_buf)
{
    ...
    if (ret < 0) {
        FTS_INFO("read touch data fails");
        return TOUCH_ERROR;
    }
    ...
    if (ts_data->suspended && ts_data->gesture_support) {
        ...
        if ((ret >= 0) && (gesture_en == ENABLE))
            ...
            FTS_INFO("gesture not enable in fw, don't process gesture");
    }
...
static int fts_irq_read_report(struct fts_ts_data *ts_data)
{
    ...
    case TOUCH_DEFAULT:
        ...
        if (finger_num > max_touch_num) {
            FTS_INFO("invalid point_num(%d)", finger_num);
            return -EIO;
        }
        ...
        for (i = 0; i < max_touch_num; i++) {
            ...
            if (pointid >= FTS_MAX_ID)
                ...
                FTS_INFO("ID(%d) beyond max_touch_number", pointid);
                return -EINVAL;
            }
    ...
    case TOUCH_EVENT_NUM:
        ...
        if (!event_num || (event_num > max_touch_num)) {
            FTS_INFO("invalid touch event num(%d)", event_num);
            return -EIO;
        }
        ...
        for (i = 0; i < event_num; i++) {
            ...
            if (pointid >= max_touch_num) {
                FTS_INFO("touch point ID(%d) beyond max_touch_number(%d)",
    ...
    case TOUCH_EXTRA_MSG:
        if (!ts_data->touch_analysis_support) {
            FTS_INFO("touch_analysis is disabled");
            return -EINVAL;
        }
...
static irqreturn_t fts_irq_handler(int irq, void *data)
{
    ...
    if ((ts_data->suspended) && (ts_data->pm_suspend)) {
        ...
        if (!ret) {
            FTS_INFO("Bus don't resume from pm(deep),timeout,skip irq");
            return IRQ_HANDLED;
        }
    }
#endif
...
static int fts_input_pen_init(struct fts_ts_data *ts_data)
{
    ...
    if (!pen_dev) {
        FTS_INFO("Failed to allocate memory for input_pen device");
        return -ENOMEM;
    }
    ...
    if (ret) {
        FTS_INFO("Input device registration failed");
...
static int fts_input_init(struct fts_ts_data *ts_data)
{
    ...
    if (!input_dev) {
        FTS_INFO("Failed to allocate memory for input device");
        return -ENOMEM;
    }
    ...
    if (ret) {
        FTS_INFO("Input-pen device registration failed");
        input_set_drvdata(input_dev, NULL);
        input_free_device(input_dev);
        input_dev = NULL;
        return ret;
...
static int fts_buffer_init(struct fts_ts_data *ts_data)
{
    ...
    if (!ts_data->touch_buf) {
        FTS_INFO("failed to alloc memory for touch buf");
        return -ENOMEM;
    }
...
static int fts_pinctrl_init(struct fts_ts_data *ts)
{
    ...
    if (IS_ERR_OR_NULL(ts->pinctrl)) {
        FTS_INFO("Failed to get pinctrl, please check dts");
    ...
    if (IS_ERR_OR_NULL(ts->pins_active)) {
        FTS_INFO("Pin state[active] not found");
    ...
    if (IS_ERR_OR_NULL(ts->pins_suspend)) {
        FTS_INFO("Pin state[suspend] not found");
    ...
    if (IS_ERR_OR_NULL(ts->pins_release)) {
        FTS_INFO("Pin state[release] not found");
...
static int fts_pinctrl_select_normal(struct fts_ts_data *ts)
{
    ...
    if (ts->pinctrl && ts->pins_active) {
        ...
        if (ret < 0) {
            FTS_INFO("Set normal pin state error:%d", ret);
        }
    }
...
static int fts_pinctrl_select_suspend(struct fts_ts_data *ts)
{
    ...
    if (ts->pinctrl && ts->pins_suspend) {
        ...
        if (ret < 0) {
            FTS_INFO("Set suspend pin state error:%d", ret);
        }
    }
...
static int fts_pinctrl_select_release(struct fts_ts_data *ts)
{
    ...
    if (ts->pinctrl) {
        if (IS_ERR_OR_NULL(ts->pins_release)) {
            ...
            if (ret < 0)
                FTS_INFO("Set gesture pin state error:%d", ret);
        }
    }
...
static int fts_power_source_ctrl(struct fts_ts_data *ts_data, int enable)
{
    ...
    if (enable) {
        if (ts_data->power_disabled) {
            FTS_INFO("regulator enable !");
            gpio_direction_output(ts_data->pdata->reset_gpio, 0);
            msleep(1);
            ret = regulator_enable(ts_data->vdd);
            if (ret) {
                FTS_INFO("enable vdd regulator failed,ret=%d", ret);
            }

            if (!IS_ERR_OR_NULL(ts_data->vcc_i2c)) {
                ret = regulator_enable(ts_data->vcc_i2c);
                if (ret) {
                    FTS_INFO("enable vcc_i2c regulator failed,ret=%d", ret);
                }
            }
        ...
        if (!ts_data->power_disabled) {
            FTS_INFO("regulator disable !");
            gpio_direction_output(ts_data->pdata->reset_gpio, 0);
            msleep(1);
            ret = regulator_disable(ts_data->vdd);
            if (ret) {
                FTS_INFO("disable vdd regulator failed,ret=%d", ret);
            }
            if (!IS_ERR_OR_NULL(ts_data->vcc_i2c)) {
                ret = regulator_disable(ts_data->vcc_i2c);
                if (ret) {
                    FTS_INFO("disable vcc_i2c regulator failed,ret=%d", ret);
                }
            }
...
static int fts_power_source_init(struct fts_ts_data *ts_data)
{
    ...
    if (IS_ERR_OR_NULL(ts_data->vdd)) {
        ...
        FTS_INFO("get vdd regulator failed,ret=%d", ret);
        return ret;
    }
    ...
    if (regulator_count_voltages(ts_data->vdd) > 0) {
        ret = regulator_set_voltage(ts_data->vdd, FTS_VTG_MIN_UV,
                                    FTS_VTG_MAX_UV);
        if (ret) {
            FTS_INFO("vdd regulator set_vtg failed ret=%d", ret);
    ...
    if (!IS_ERR_OR_NULL(ts_data->vcc_i2c)) {
        if (regulator_count_voltages(ts_data->vcc_i2c) > 0) {
            ret = regulator_set_voltage(ts_data->vcc_i2c,
                                        ...
                                        FTS_I2C_VTG_MAX_UV);
            if (ret) {
                FTS_INFO("vcc_i2c regulator set_vtg failed,ret=%d", ret);
    ...
    if (ret) {
        FTS_INFO("fail to enable power(regulator)");
    }
...
static int fts_power_source_suspend(struct fts_ts_data *ts_data)
{
    ...
    if (ret < 0) {
        FTS_INFO("power off fail, ret=%d", ret);
    }
...
static int fts_power_source_resume(struct fts_ts_data *ts_data)
{
    ...
    ret = fts_power_source_ctrl(ts_data, ENABLE);
    if (ret < 0) {
        FTS_INFO("power on fail, ret=%d", ret);
    }
...
static int fts_gpio_configure(struct fts_ts_data *data)
{
    ...
    if (gpio_is_valid(data->pdata->irq_gpio)) {
        ...
        if (ret) {
            FTS_INFO("[GPIO]irq gpio request failed");
            goto err_irq_gpio_req;
        }

        ret = gpio_direction_input(data->pdata->irq_gpio);
        if (ret) {
            FTS_INFO("[GPIO]set_direction for irq gpio failed");
    ...
    if (gpio_is_valid(data->pdata->reset_gpio)) {
        ...
        if (ret) {
            FTS_INFO("[GPIO]reset gpio request failed");
            goto err_irq_gpio_dir;
        }

        ret = gpio_direction_output(data->pdata->reset_gpio, 1);
        if (ret) {
            FTS_INFO("[GPIO]set_direction for reset gpio failed");
                             ...
                             struct fts_ts_platform_data *pdata)
{
    ...
    if (coords_size != FTS_COORDS_ARR_SIZE) {
        FTS_INFO("invalid:%s, size:%d", name, coords_size);
        return -EINVAL;
    }

    ret = of_property_read_u32_array(np, name, coords, coords_size);
    if (ret < 0) {
        FTS_INFO("Unable to read %s, please check dts", name);
...
static int fts_parse_dt(struct device *dev, struct fts_ts_platform_data *pdata)
{
    ...
    if (ret < 0)
        FTS_INFO("Unable to get display-coords");
    ...
    if (pdata->have_key) {
        ...
        if (ret < 0)
            FTS_INFO("Key number undefined!");

        ret = of_property_read_u32_array(np, "focaltech,keys",
                                         pdata->keys, pdata->key_number);
        if (ret < 0)
            FTS_INFO("Keys undefined!");
        ...
        ret = of_property_read_u32_array(np, "focaltech,key-x-coords",
                                         ...
                                         pdata->key_number);
        if (ret < 0)
            FTS_INFO("Key Y Coords undefined!");

        ret = of_property_read_u32_array(np, "focaltech,key-y-coords",
                                         pdata->key_y_coords,
                                         pdata->key_number);
        if (ret < 0)
            FTS_INFO("Key X Coords undefined!");
    ...
    if (pdata->reset_gpio < 0)
        FTS_INFO("Unable to get reset_gpio");

    pdata->irq_gpio = of_get_named_gpio_flags(np, "focaltech,irq-gpio",
                      0, &pdata->irq_gpio_flags);
    if (pdata->irq_gpio < 0)
        FTS_INFO("Unable to get irq_gpio");

    ret = of_property_read_u32(np, "focaltech,max-touch-number", &temp_val);
    if (ret < 0) {
        FTS_INFO("Unable to get max-touch-number, please check dts");
...
static int drm_check_dt(struct device_node *np)
{
    ...
    if (count <= 0) {
        FTS_INFO("find drm_panel count(%d) fail", count);
        return -ENODEV;
    }
...
static int fts_ts_probe_entry(struct fts_ts_data *ts_data)
{
    ...
    if (!ts_data->pdata) {
        FTS_INFO("allocate memory for platform_data fail");
        return -ENOMEM;
    }
    ...
    if (ts_data->dev->of_node) {
        ...
        if (ret)
            FTS_INFO("device-tree parse fail");

#if defined(CONFIG_DRM)
#if defined(FOCAL_CONFIG_DRM_PANEL)
        ret = drm_check_dt(ts_data->dev->of_node);
        if (ret) {
            FTS_INFO("parse drm-panel fail");
        }
#endif
#endif
    } else {
        if (ts_data->dev->platform_data) {
            ...
            FTS_INFO("platform_data is null");
            return -ENODEV;
        }
    }
    ...
    if (!ts_data->ts_workqueue) {
        FTS_INFO("create fts workqueue fail");
    }
    ...
    if (ret) {
        FTS_INFO("bus initialize fail");
        goto err_bus_init;
    }

    ret = fts_input_init(ts_data);
    if (ret) {
        FTS_INFO("input initialize fail");
        goto err_input_init;
    }

    ret = fts_buffer_init(ts_data);
    if (ret) {
        FTS_INFO("buffer init fail");
        goto err_buffer_init;
    }

    ret = fts_gpio_configure(ts_data);
    if (ret) {
        FTS_INFO("configure the gpios fail");
    ...
    if (ret) {
        FTS_INFO("fail to get power(regulator)");
        goto err_power_init;
    }
#endif
    ...
    if (ret) {
        FTS_INFO("not focal IC, unregister driver");
        goto err_irq_req;
    }

    ret = fts_create_apk_debug_channel(ts_data);
    if (ret) {
        FTS_INFO("create apk debug node fail");
    }

    ret = fts_create_sysfs(ts_data);
    if (ret) {
        FTS_INFO("create sysfs node fail");
    }

    ret = fts_point_report_check_init(ts_data);
    if (ret) {
        FTS_INFO("init point report check fail");
    }

    ret = fts_ex_mode_init(ts_data);
    if (ret) {
        FTS_INFO("init glove/cover/charger fail");
    }

    ret = fts_gesture_init(ts_data);
    if (ret) {
        FTS_INFO("init gesture fail");
    }


    ret = fts_esdcheck_init(ts_data);
    if (ret) {
        FTS_INFO("init esd check fail");
    }

    ret = fts_irq_registration(ts_data);
    if (ret) {
        FTS_INFO("request irq failed");
        goto err_irq_req;
    }

    ret = fts_fwupg_init(ts_data);
    if (ret) {
        FTS_INFO("init fw upgrade fail");
    }
    ...
    if (ret) {
        FTS_INFO("[FB]Unable to register fb_notifier: %d", ret);
    }
#elif defined(CONFIG_DRM)
    ...
    if (active_panel) {
        ...
        if (ret)
            FTS_INFO("[DRM]drm_panel_notifier_register fail: %d\n", ret);
    }
#else
    ret = msm_drm_register_client(&ts_data->fb_notif);
    if (ret) {
        FTS_INFO("[DRM]Unable to register fb_notifier: %d\n", ret);
    }
#endif
#elif defined(CONFIG_HAS_EARLYSUSPEND)
...
static int fts_ts_remove_entry(struct fts_ts_data *ts_data)
{
    ...
    if (fb_unregister_client(&ts_data->fb_notif))
        FTS_INFO("[FB]Error occurred while unregistering fb_notifier.");
#elif defined(CONFIG_DRM)
#if defined(FOCAL_CONFIG_DRM_PANEL)
    if (active_panel)
    ...
    if (msm_drm_unregister_client(&ts_data->fb_notif))
        FTS_INFO("[DRM]Error occurred while unregistering fb_notifier.\n");
#endif
#elif defined(CONFIG_HAS_EARLYSUSPEND)
...
static int fts_ts_suspend(struct device *dev)
{
    ...
    if (ts_data->gesture_support) {
        ...
        if (ret < 0)
            FTS_INFO("set TP to sleep mode fail, ret=%d", ret);

        if (!ts_data->ic_info.is_incell) {
#if FTS_POWER_SOURCE_CUST_EN
            ret = fts_power_source_suspend(ts_data);
            if (ret < 0) {
                FTS_INFO("power enter suspend fail");
            }
#endif
        }
    }
...
* TP Driver
*****************************************************************************/
static void focaltech_pdev_release(struct device *dev)
{
	FTS_INFO("goodix pdev released");
	kfree(focaltech_pdev);
}
...
static int fts_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    if (!ts_data) {
        FTS_INFO("allocate memory for fts_data fail");
        return -ENOMEM;
    }
    ...
    if (ret) {
        FTS_INFO("Touch Screen(I2C BUS) driver probe fail");
        kfree_safe(ts_data);
        return ret;
    }
	
	focaltech_pdev = kzalloc(sizeof(struct platform_device), GFP_KERNEL);
	if (!focaltech_pdev)
		return -ENOMEM;

	focaltech_pdev->name = FTS_DRIVER_NAME;
	focaltech_pdev->id = 0;
	focaltech_pdev->num_resources = 0;
	/*
	 * you can find this platform dev in
	 * /sys/devices/platform/goodix_ts.0
	 * focaltech_pdev->dev.parent = &client->dev;
	 */
	focaltech_pdev->dev.platform_data = &ts_data;
	focaltech_pdev->dev.release = focaltech_pdev_release;

	/* register platform device, then the goodix_ts_core
	 * module will probe the touch device.
	 */
	ret = platform_device_register(focaltech_pdev);
	if (ret) {
		FTS_INFO("failed register goodix platform device, %d", ret);
		goto err_pdev;
	}
...
err_pdev:
	kfree(focaltech_pdev);
	focaltech_pdev = NULL;
	FTS_INFO("i2c probe out, %d", ret);
	return ret;
}

static int fts_ts_remove(struct i2c_client *client)
{
	platform_device_unregister(focaltech_pdev);
...
static const struct platform_device_id fts_core_ids[] = {
	{.name = FT_CORE_DRIVER_NAME},
	{}
};
MODULE_DEVICE_TABLE(platform, fts_core_ids);

static struct platform_driver fts_platform_driver = {
	.driver = {
		.name = FT_CORE_DRIVER_NAME,
		.owner = THIS_MODULE,
	},
...
static int __init fts_ts_init(void)
{
    ...
    if ( ret != 0 ) {
        FTS_INFO("Focaltech touch screen driver init failed!");
		return ret;
    }
    FTS_FUNC_EXIT();
    return platform_driver_register(&fts_platform_driver);
}

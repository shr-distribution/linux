...
#define PULSE_PSOC_CS_TOGGLE 25

/* Since the PSoC runs a lot slower than the host, it needs
 * some time to get ready after the chip select gets
 * asserted. Using an empty transfer, insert 20 usecs of
 * delay after CS assertion. Even though 18 usecs seems
 * to be a sufficient delay, extra few usecs will help
 * with reliability without impacting the performance.
 */
#define CS_ASSERT_DELAY     20
/*
 *
 */
#define CS_DEASSERT_DELAY    5
...
cy8mrln_reg_write(struct tsc_drv_data *dev, u16 reg, u8 data)
{
	...
	/* send fake tx to trigger cs assert delay */
	t[0].bits_per_word = 8;
	t[0].tx_buf = NULL;
	t[0].rx_buf = NULL;
	t[0].len    = 0;
	t[0].delay_usecs = CS_ASSERT_DELAY;
	...
	t[1].rx_buf = rx;
	t[1].len    = 3;
	t[1].delay_usecs = CS_DEASSERT_DELAY;
...
{   
	...
	t.rx_buf = rxbuf;
	t.len    = length;
	t.delay_usecs = CS_DEASSERT_DELAY;
...
cy8mrln_work_handler_new(struct work_struct *work)
{
	    ...
	    CY8MRLN_WOT_RAW_DATA == dev->cmd    ) 
	{
		...
		//
		if( rxbuf->ptr[0] != 0x0D || 
			((rxbuf->ptr[1] != 0x0A) && (rxbuf->ptr[1] != 0x0B) ) )
		{
			// Bad Frame Marker aka SOF or EOF
			//
			printk(KERN_INFO "%s: Bad SOF:Got[0x%02x,0x%02x] " \
				"Expected[0x0D, 0x0A or 0x0B](SCN)\n",
				DRIVER, rxbuf->ptr[0], rxbuf->ptr[1] );
			// TODO: Don't copy to Rx Buff -- bad packet.
		}
		
		if( dev->wot_rawdata_mode == WOT_DBG_OFF_PENDING )
		{
			/* Eject from WOT Raw Data Mode */
			cy8mrln_toggle_cs(dev, 30) ; 	//Already have the interrupt now just have 
			                                //to toggle cs to eject from from wot raw data mode.
			printk(KERN_INFO"%s: Ejected from WOT rawdata mode\n", DRIVER);
			dev->wot_rawdata_mode = WOT_DBG_OFF ;
			dev->auto_scan        = 0 ;
		}
		...
		if(	dev->auto_scan == 1 && /* WOT is enabled    */
			...
			dev->wot_rawdata_mode == WOT_DBG_OFF  /* no need to issue wot scan in raw mode which is self-issuing */
			)
		{
			u16 regval, sensval;
		...
		if( dev->wot_rawdata_mode == WOT_DBG_ON_PENDING )
		{
			/* Flush read buffer to avoid stale data upon wake up */
			cy8mrln_flush_rd_buffers( dev );
			/* Enter raw data mode immediately */
			cy8mrln_reg_write( dev, 0, CY8MRLN_WOT_RAW_DATA );
			dev->wot_rawdata_mode = WOT_DBG_ON; // 1->2 transition
			printk(KERN_INFO "%s:spi cmd sent to psoc wot_rawdata_mode\n", DRIVER );
			dev->auto_scan = 1 ;
			/* Cancel timers */
#ifdef CONFIG_HIGH_RES_TIMERS
			hrtimer_cancel(&dev->scan_timer);
              ...
              unsigned int cmd, unsigned long args)
{
	...
	switch (cmd) {
		...
		case CY8MRLN_IOCTL_ENTER_WOT_RAW_DATA:
		{
			printk("%s: ENTER_WOT_RAW_DATA\n", DRIVER);
			dev->wot_rawdata_mode = WOT_DBG_ON_PENDING ;
			dev->auto_scan = 1 ;
		} break;

		case CY8MRLN_IOCTL_EXIT_WOT_RAW_DATA:
		{
			printk("%s: EXIT_WOT_RAW_DATA\n", DRIVER);
			dev->wot_rawdata_mode = WOT_DBG_OFF_PENDING ; // signal to eject from WOT raw data mode
			dev->auto_scan = 0 ;
		} break;
...
/*
 * Description of WOT RAW DATA mode (from Cypress)
 *
(1) PSoC in IDLE.
(2) Palm Host issues a ‘WOT Raw’ command: 0x00-0x00-0x0F
(3) PSoC will then:

            a) Enter WOT-Raw Mode.
            b) Auto-Scan the WOT Zones.
            c) Construct a 158-Byte Scan Packet structured as:
                        0x0D0B-….{garbage}…-WOTrawzone1-WOTrawzone2-WOTrawzone3-0xFFFF
                        Notes:
The 6-Bytes of raw WOT data are the last 6-bytes of the packet, followed by the framing-end bytes 0xFFFF.
The Header is 0x0D0B for Raw WOT Data (0x0D0A for Scan data, 0x0D0C for IDAC data).
            d) Generate an IRQ to the Palm Host.
            e) Wait in the normal scan data-phase for 158-Bytes of SCLK.
(4) Upon completion of the 158 byte SCLK data-phase, the PSoC will then Execute a WOT Sleep cycle, and upon wake-up…
Loop to (3b) above.
…and so on.

To eject from this mode the Palm Host should:
(1)     Receive the IRQ from the PSoC.
(2)     Assert, then De-Assert CS (no SCLKs needed).

This will cause the PSoC to:
(1)     Abort the data-phase (same as when in normal scan mode).
(2)     Exit WOT-Raw mode.
(3)     Return to IDLE.

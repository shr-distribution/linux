...
#define FTS_GESTRUE_EN      1//0:disable, 1: enable
...
#if 1 //defined(CONFIG_BBRY) || defined(BBRY_MINISW)
...
static void fts_gesture_report(struct input_dev *input_dev,int gesture_id)
{
	FTS_COMMON_DBG("fts gesture_id=0x%02x", gesture_id);
...
static int fts_gesture_read_data(void)
{
    ...
    if (ret < 0)
    {
        FTS_COMMON_DBG( "%s read touchdata failed.",  __func__);
        return ret;
    }

    /* FW */
    if (fts_upgrade_info_curr.chip_id==0x54
		|| fts_upgrade_info_curr.chip_id==0x58
		|| fts_upgrade_info_curr.chip_id==0x86
		|| fts_upgrade_info_curr.chip_id==0x87
		)
    {
		gestrue_id = buf[0];
	...
	{
		FTS_COMMON_DBG( "%s: IC 0x%x need gesture lib to support gestures.",  __func__, fts_upgrade_info_curr.chip_id);
		return -1;
	}
}

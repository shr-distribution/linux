...
//static ssize_t fts_proc_read( char *page, char **start, off_t off, int count, int *eof, void *data );
static ssize_t fts_proc_read (struct file *filp, char __user *buff, size_t count, loff_t *ppos);
#else
static int fts_proc_write(struct file *filp, const char __user *buff, unsigned long len, void *data);
...
int fts_apk_node_init(void)
{
	...
	FTS_COMMON_DBG("[focal] %s ",  FOCALTECH_APK_NODE_INFO);	//show version
...
* Brief: i2c read, read data from TP
...
* Brief: i2c write, write data to TP
* Input: i2c info, data, length
* Output: no
* Return: fail <0
***********************************************************************/
...
static int fts_char_device_init(void)
{
	...
	if (err < 0)
	{
		FTS_COMMON_DBG( " ft_rw_iic_drv failed  error code=%d---",  err);
		return err;
	}
	...
	/*	register a class and mdev can create device node in /dev/ directory.	*/
...
static ssize_t fts_proc_write(struct file *filp, const char __user *buff, size_t count, loff_t *ppos)
{
	...
	if (copy_from_user(&writebuf, buff, buflen)) {
		FTS_COMMON_DBG( "%s:copy from user error",  __func__);
		return -EFAULT;
	}
	...
	case PROC_UPGRADE:
		{
			...
			FTS_COMMON_DBG("%s",  upgrade_file_path);
			...
			if (ret < 0) {
				FTS_COMMON_DBG( "%s:upgrade failed.",  __func__);
				return ret;
			}
		}
		break;
	...
	case PROC_READ_REGISTER:
		...
		if (ret < 0) {
			FTS_COMMON_DBG( "%s:write iic error",  __func__);
			return ret;
		}
		break;
	case PROC_WRITE_REGISTER:
		...
		break;
	case PROC_AUTOCLB:
		FTS_COMMON_DBG("%s: autoclb",  __func__);
		fts_ctpm_auto_clb();
		break;
	case PROC_READ_DATA:
...
static ssize_t fts_proc_read (struct file *filp, char __user *buff, size_t count, loff_t *ppos)
{
	...
	case PROC_READ_REGISTER:
		...
		if (ret < 0) {
			FTS_COMMON_DBG( "%s:read iic error",  __func__);
			return ret;
		}
		num_read_chars = 1;
		break;
	...
	if (copy_to_user(buff, buf, num_read_chars)) {
		FTS_COMMON_DBG( "%s:copy to user error",  __func__);
		return -EFAULT;
	}
...
*   Name: fts_proc_read
*  Brief:interface of read proc
* page: address of writting data
* count: max char number that can be writed into page address
* set value of start and off when there're multi pages data.(One page size normal is 4KB)
*  Need to set eof (end of file) after writed all data.
*  data: private data.
* Not need to use copy_to_user because page address is in kernel.
* Input: point to the data, no use, no use, read len, no use, no use
* Output: page point to data
* Return: read char number
***********************************************************************/

				...
				int brightness)
{
	...
	//msx
	printk("mtk_set_hw_brightness brightness value 111111 is %d\n",brightness);	
	
	brightness = min(brightness, 80);
	
	if(brightness_flag==2)
		brightness = 0;
		
	printk("mtk_set_hw_brightness brightness value 222222 is %d\n",brightness);
	//end
...
int mt_leds_parse_dt(struct mt_led_data *mdev, struct fwnode_handle *fwnode)
{
	...
	mdev->conf.cdev.max_brightness = 80;
	...
	mdev->conf.max_hw_brightness = 80;
	...
	printk("parse led: %s, num: %d, connector_id: %d, max: %d, min: %d, max_hw: %d, brightness: %d",
		mdev->conf.cdev.name,
		leds_info->lens,

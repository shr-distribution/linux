...
#include "../../../../power/supply/cps_wls_charger.h"
#endif
	...
	case TCP_NOTIFY_TYPEC_STATE:
		...
		if (old_state == TYPEC_UNATTACHED &&
			...
			dev_info(rpmd->dev,
				 ...
				 __func__, noti->typec_state.polarity);
#ifdef CONFIG_CPS_WLS_CHARGER		
			//无线充接入的时候不允许打开开关给otg供电，否则10V电压会把OTG烧坏
			tcpci_get_vbus_voltage(rpmd->tcpc, &vbus);
			dev_info(rpmd->dev, "%s OTG plug in check vbus:%d\n", __func__, vbus);
			if((wls_mod0_gp1_get() == 0) && (vbus == 0)){
				dev_info(rpmd->dev, "%s OTG plug in, set ovp flag 1\n", __func__);
				wls_charger_ovp_flag_set(1);
				wls_mod1_gp2_set(1);//disable wireless charger output
#endif							
			...
			typec_set_pwr_role(rpmd->typec_port, TYPEC_SOURCE);
				...
				dev_info(rpmd->dev, "%s wireless charger plug in, do not source otg\n", __func__);
			}
#endif
...
#ifdef CONFIG_CPS_WLS_CHARGER
				wls_charger_ovp_flag_set(0);
				wls_mod1_gp2_set(0);//disable wireless charger output
#endif

...
bool backlight_limilt_flag=1;// yqf add
bool kpd_suspend_flag = 0;// yqf add
...
int disp_lcm_suspend(struct disp_lcm_handle *plcm)
{
	...
	if (_is_lcm_inited(plcm)) {
		...
		// printk("[yqf] disp_lcm_suspend\r\n");
		backlight_limilt_flag = 0;	
		DISPMSG("[yqf] kpd disp_lcm_suspend\n");
        kpd_suspend_flag = 1;
		return 0;
	}
	{
...
int disp_lcm_resume(struct disp_lcm_handle *plcm)
{
	...
	if (_is_lcm_inited(plcm)) {
        ...
        DISPMSG("[yqf] kpd disp_lcm_resume\n");
		kpd_suspend_flag = 0;
		return 0;
	}
	{

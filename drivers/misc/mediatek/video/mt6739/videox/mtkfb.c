...
UINT32 color;
/*yqf add it for along device info*/
#if 1
#define RGT_INFO_NAME  "alonginfo"
static struct class *rgt_info_class=NULL;
static struct device	*rgt_dev=NULL;

static char front_camera_name[256] = "no";
static char back_camera_name[256] = "no";
static char g_sensor_name[256] = "no";
static char gyro_sensor_name[256] = "no";
static char m_sensor_name[256] = "no";
static char rgt_lcm_name[256] = "no";
static char ctp_name[256] = "no";
static char als_ps_sensor_name[256] = "no";

void app_get_rgt_lcm_name(char *name)
{
     if(name!=NULL)
        strcpy(rgt_lcm_name,name);
}
EXPORT_SYMBOL(app_get_rgt_lcm_name); 

void app_get_ctp_name(char *name)  
{
     if(name!=NULL)
...
void app_get_back_sensor_name(char *back_name)  //back_camera
{
     if(back_name!=NULL)
...
void app_get_front_sensor_name(char *front_name) //front_camera
{	 
     if(front_name!=NULL)	 
...
void app_get_gyro_sensor_name(char *gyro_name) //gyro_sensor
{
     if(gyro_name!=NULL)
       strcpy(gyro_sensor_name,gyro_name);
}
EXPORT_SYMBOL(app_get_gyro_sensor_name); 
...
static ssize_t rgt_info_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t size)
{
	return 0;
}
		
static ssize_t rgt_info_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
 ...
 // lcd :
      ...
      i+=sprintf(temp, "[ lcd=%s ]\n", rgt_lcm_name);  
       strcat(buf, temp);

// back camera:
	  memset(temp, 0, sizeof(temp));
	  i+=sprintf(temp, "[ back camera=%s ]\n", back_camera_name);  
	  strcat(buf, temp); 		
	 
 // front camera:
      memset(temp, 0, sizeof(temp));
      i+=sprintf(temp, "[ front camera=%s ]\n", front_camera_name);  
       strcat(buf, temp);	   

 // gsensor:
      memset(temp, 0, sizeof(temp));
      i+=sprintf(temp, "[ gsensor=%s ]\n", g_sensor_name);  
       strcat(buf, temp);

 // gyrosensor:
      memset(temp, 0, sizeof(temp));
      i+=sprintf(temp, "[ gyrosensor=%s ]\n", gyro_sensor_name);  
       strcat(buf, temp);	

 // msensor:
      memset(temp, 0, sizeof(temp));
      i+=sprintf(temp, "[ msensor=%s ]\n", m_sensor_name);  
       strcat(buf, temp);	   
...
static DEVICE_ATTR(along_info, S_IRUGO|S_IWUSR, rgt_info_show, rgt_info_store);  //add by jun.zhou for show device's name
// system/class/alonginfo/alongv0/along_info
void  rgt_info_init(void)
{

	rgt_info_class = class_create(THIS_MODULE, RGT_INFO_NAME);
	rgt_dev = device_create(rgt_info_class, NULL, 0, 0,"alongv0");
	if (IS_ERR(rgt_dev)) {
		printk(KERN_DEBUG
		       "rgt_info_init: device_create "
		       "failed (%ld)\n", PTR_ERR(rgt_dev));
		return;
	}
	device_create_file(rgt_dev, &dev_attr_along_info);  //add by yqf for show device's name

}

void  rgt_info_deinit(void)
{

       device_remove_file(rgt_dev, &dev_attr_along_info); //add by yqf for show device's name
       device_unregister(rgt_dev);
	   
	if(rgt_info_class!=NULL)
	       class_destroy(rgt_info_class);
	
}

#endif
/*yqf add it for passion device info end*/
...
char *mtkfb_find_lcm_driver(void)
{
	...
	app_get_rgt_lcm_name(mtkfb_lcm_name);
...
static int mtkfb_probe(struct platform_device *pdev)
{
    ...
    rgt_info_init();//yqf add it for devices show
	MSG_FUNC_LEAVE();
	return 0;

		       ...
		       enum CMDQ_STATE state)
{
	...
	if (state == CMDQ_WAIT_LCM_TE) {
		...
		for (i = 0; i < 3; i++) {
			...
			/* 2. save RX data */
			if (hSlot) {
				 DSI_BACKUPREG32(cmdq_trigger_handle, hSlot, i * 4 + 0, &DSI_REG[0]->DSI_RX_DATA0);
                    DSI_BACKUPREG32(cmdq_trigger_handle, hSlot, i * 4 + 1, &DSI_REG[0]->DSI_RX_DATA1);
                    DSI_BACKUPREG32(cmdq_trigger_handle, hSlot, i * 4 + 2, &DSI_REG[0]->DSI_RX_DATA2);
                    DSI_BACKUPREG32(cmdq_trigger_handle, hSlot, i * 4 + 3, &DSI_REG[0]->DSI_RX_DATA3);
			}
			/* 3. write RX_RACK */
	  ...
	  DISPDBG("[DSI]enter cmp read_data3 byte0=0x%x byte1=0x%x byte2=0x%x byte3=0x%x \n",read_data3.byte0,read_data3.byte1,read_data3.byte2,read_data3.byte3);
			...
			if (packet_type == 0x1A || packet_type == 0x1C) {
                    ...
                    if (recv_data_cnt > 2) {
                            DISPDBG("Set receive data count from %d to 2 as ESD check supported max data count.\n", recv_data_cnt);
                            recv_data_cnt = 2;
                    }
                    if (recv_data_cnt > dsi_params->lcm_esd_check_table[i].count) {
                            DISPDBG("Set receive data count from %d to %d as ESD check table specified.\n",

...
extern bool backlight_limilt_flag;
int mt_mt65xx_led_set_cust(struct cust_mt65xx_led *cust, int level)
{
	...
	//printk("brightness_set_cust:backlight_limilt_flag = %d,cust->mode %d,cust->name %s!!!!!\n",backlight_limilt_flag,cust->mode,cust->name);
	if (strcmp(cust->name, "lcd-backlight") == 0)
	{
		if(!backlight_limilt_flag)
		{
			mdelay(60);// yqf modify 100
			backlight_limilt_flag = 1;
		}
	}

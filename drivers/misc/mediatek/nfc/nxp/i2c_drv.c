...
int nfc_i2c_dev_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	if (ret <= 0) {
		printk("%s: unable to request nfc irq gpio [%d]\n", __func__,
		       nfc_gpio->irq);
		goto err_free_gpio;
	}
	client->irq = ret;
	...
	if (ret) {
		printk("%s: unable to request nfc firm downl gpio [%d]\n",
	...
	if (ret) {
		printk("%s: nfc_misc_register failed\n", __func__);
	...
	printk("%s: probing nfc i2c successfully\n", __func__);
	return 0;
...
err_free_nfc_dev:
	kfree(nfc_dev);
err:
	printk("%s: probing not successful, check hardware\n", __func__);
	return ret;
}

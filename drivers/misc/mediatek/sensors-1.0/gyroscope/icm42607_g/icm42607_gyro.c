...
#include "gyroscope.h"
#include "../../accelerometer/icm42607_a/icm42607_register.h"
#include "../../accelerometer/icm42607_a/icm42607_share.h"
...
#define GYR_DEBUG_ON          		0

#if GYR_DEBUG_ON
/* Log define */
#define GYR_INFO(fmt, arg...)      	printk("<<-GYR INFO->> "fmt"\n", ##arg)
#define GYR_ERR(fmt, arg...)          	printk("<<-GYR ERROR->> "fmt"\n", ##arg)
#define GYR_FUNC(fmt, arg...)      	printk("<<-GYR FUNC->> Func:%s@Line:%d\n", __func__, __LINE__);
#else
//#define GSE_TAG
#define GYR_INFO(fmt, args...)	do {} while (0)
#define GYR_ERR(fmt, args...)	do {} while (0)
...
static int icm42607_gyro_SetFullScale(struct i2c_client *client, u8 gyro_fsr)
{
    ...
    if (res < 0){
        GYR_ERR("write fsr register err!\n");
        return ICM42607_ERR_BUS;
    }
...
static int icm42607_gyro_SetFilter(struct i2c_client *client, u8 gyro_filter)
{
    ...
    if (res < 0) {
        GYR_ERR("read filter register err!\n");
        return ICM42607_ERR_BUS;
    }
    /* clear filter bit */
    ...
    if (res < 0) {
        GYR_ERR("write filter register err!\n");
        return ICM42607_ERR_BUS;
    }
    return ICM42607_SUCCESS;
}
    ...
    s16 data[ICM42607_AXIS_NUM])
{
    ...
    res = icm42607_share_read_register(REG_GYRO_DATA_X0_UI, databuf,
        ICM42607_DATA_LEN);
    if (res < 0) {
        GYR_ERR("read gyroscope data error\n");
        return ICM42607_ERR_BUS ;
    }
    ...
    char *buf, int bufsize)
{
    ...
    if (atomic_read(&obj->trace)) {
        GYR_INFO("Gyroscope data - %04x %04x %04x\n",
            data[ICM42607_AXIS_X],
            data[ICM42607_AXIS_Y],
            data[ICM42607_AXIS_Z]);
    }
    ...
    struct SENSOR_DATA *sensor_data)
{
    ...
    if (atomic_read(&obj->trace)) {
        GYR_INFO("Gyro ReadCalibration:[sensor_data:%5d %5d %5d]\n",
            sensor_data->x, sensor_data->y, sensor_data->z);
        GYR_INFO("Gyro ReadCalibration:[cali_sw:%5d %5d %5d]\n",
            obj->cali_sw[ICM42607_AXIS_X],
            obj->cali_sw[ICM42607_AXIS_Y],
            obj->cali_sw[ICM42607_AXIS_Z]);
    }
...
#ifdef DEBUG
	GYR_INFO("write gyro calibration data  (%5d, %5d, %5d)-->(%5d, %5d, %5d)\n",
			sensor_data ->x, sensor_data ->y, sensor_data ->z,
...
static bool icm42607_gyro_DoSelfTest(struct i2c_client *client)
{
    ...
    for (retry = 0 ; retry < RETRY_CNT_SELF_TEST ; retry++ ) {
        ...
        res = icm42607_share_write_blkreg(BIT_BLK_SEL_1, REG_SELF_TEST_CONFIG, databuf, 1);
        if (res) {
            GYR_ERR("disable st gyro fail: %d\n", res);
            return false;
        }
        /* wait 20ms for oscillations to stabilize */
        mdelay(20);
        ...
        if (!otp_value_has_zero) {
            ...
            for (i = 0; i < ICM42607_AXIS_NUM; i++) {
                ...
                if (st_res <= st_otp[i] * SELF_TEST_GYR_SHIFT_DELTA) {
                    GYR_INFO("error gyro[%d] : st_res = %d, st_otp = %d\n",
                        i, st_res, st_otp[i]);
                    test_result = false;
                }
            }
            ...
            for (i = 0; i < ICM42607_AXIS_NUM; i++) {
                ...
                if (st_res < SELF_TEST_MIN_GYR) {
                    GYR_INFO("error gyro[%d] : st_res = %d, min = %d\n",
...
static int icm42607_gyro_init_client(struct i2c_client *client, bool enable)
{
    ...
    if (res != ICM42607_SUCCESS)
        return res;
    GYR_INFO("icm42607_gyro_init_client OK!\n");
    return ICM42607_SUCCESS;
}
    ...
    const char *buf, size_t count)
{
    ...
    if (0 == kstrtoint(buf, 16, &trace))
        ...
        GYR_ERR("invalid content: '%s', length = %zu\n", buf, count);
    return count;
}
...
static ssize_t show_chip_orientation(struct device_driver *ddri, char *buf)
{
    ...
    GYR_INFO("[%s] default direction: %d\n", __func__, obj->hw->direction);
    ...
    const char *buf, size_t tCount)
{
    ...
    if (res != 0) {
        if (hwmsen_get_convert(nDirection, &obj->cvt))
            GYR_ERR("ERR: fail to set direction\n");
    }
    GYR_INFO("[%s] set direction: %d\n", __func__, nDirection);
    return tCount;
}
    ...
    const char *buf, size_t count)
{
    ...
    if (res != 0) {
        GYR_ERR("parse number fail\n");
        return count;
    } else if (num == 0) {
        GYR_ERR("invalid data count\n");
        return count;
    }
    /* run selftest */
    ...
    if (icm42607_gyro_DoSelfTest(client) == true) {
        ...
        GYR_INFO("GYRO SELFTEST : PASS\n");
    } else {
        strcpy(selftestRes, "n");
        GYR_INFO("GYRO SELFTEST : FAIL\n");
    }
...
static int icm42607_gyro_create_attr(struct device_driver *driver)
{
    ...
    for (idx = 0; idx < num; idx++) {
        ...
        if (0 != res) {
            GYR_ERR("driver_create_file (%s) = %d\n",
...
static int icm42607_gyro_ReadRawData(struct i2c_client * client, char * buf)
{
    ...
    if (res < 0) {
        GYR_ERR("read gyroscope raw data  error\n");
        return ICM42607_ERR_BUS;
    }
    ...
    unsigned int cmd, unsigned long arg)
{
    ...
    if (res) {
        GYR_ERR("access error: %08X, (%2d, %2d)\n",
    ...
    case GYROSCOPE_IOCTL_SMT_DATA:
        ...
        GYR_INFO("ioctl smtRes: %d!\n", smtRes);
        copy_cnt = copy_to_user(data, &smtRes,  sizeof(smtRes));
        if (copy_cnt) {
            res = -EFAULT;
            GYR_ERR("copy gyro data to user failed!\n");
        }
        GYR_INFO("copy gyro data to user OK: %d!\n", copy_cnt);
        break;
    ...
    case GYROSCOPE_IOCTL_SET_CALI:
        ...
        if (copy_from_user(&sensor_data, data, sizeof(sensor_data)))
            ...
            GYR_INFO("gyro set cali:[%5d %5d %5d]\n",
                sensor_data.x, sensor_data.y, sensor_data.z);
    ...
    unsigned int cmd, unsigned long arg)
{
    ...
    if(!file->f_op || !file->f_op->unlocked_ioctl){
        GYR_ERR("compat_ion_ioctl file has no f_op\n");
        GYR_ERR("or no f_op->unlocked_ioctl.\n");
        return -ENOTTY;
    }
    switch (cmd) {
...
static int icm42607_gyro_factory_get_cali(int32_t data[3])
{
        ...
        if (err) {
                GYR_INFO("icm42607_gyro_ReadCalibration failed!\n");
                return -1;
        }
...
static int icm42607_gyro_factory_set_cali(int32_t data[3])
{
        ...
        if (err) {
                GYR_INFO("406xx_gyro_WriteCalibration failed!\n");
                return -1;
        }
        return 0;
}
...
static int icm42607_gyro_factory_clear_cali(void)
{
        ...
        if (err) {
                GYR_INFO("icm42607_gyro_factory_clear_cali failed!\n");
                return -1;
        }
        return 0;
}
...
static int icm42607_gyro_factory_get_raw_data(int32_t data[3])
{
        ...
        GYR_INFO("do not support raw data now!\n");
        return 0;
}
...
static int icm42607_gyro_factory_enable_sensor(bool enabledisable, int64_t sample_periods_ms)
{
        ...
        if (err) {
                GYR_INFO("%s enable gyro sensor failed!\n", __func__);
                return -1;
        }
        return 0;
}
...
static int icm42607_gyro_enable_nodata(int en)
{
    ...
    if (res != ICM42607_SUCCESS) {
        GYR_ERR("icm42607_gyro_SetPowerMode fail!\n");
        return -1;
    }
    atomic_set(&obj->is_enabled, en);
    GYR_INFO("icm42607_gyro_enable_nodata OK!\n");
    return 0;
}
...
static int icm42607_gyro_get_data(int *x , int *y, int *z, int *status)
{
    ...
    icm42607_gyro_ReadSensorData(obj_i2c_data->client, buff, ICM42607_BUFSIZE);
	 ...
	 if (res < 0) {
     	GYR_ERR("read gyro data not ready\n");
        return ICM42607_ERR_BUS;
    }
    ...
    const struct i2c_device_id *id)
{
	...
	res = get_gyro_dts_func(client->dev.of_node, hw);
        if (res < 0) {
                GYR_ERR("get gyro dts info fail\n");
                goto exit;
        }
    ...
    if (res) {
        GYR_ERR("invalid direction: %d\n", obj->hw->direction);
        goto exit;
    }
    GYR_INFO("direction: %d,addr: 0x%x\n", obj->hw->direction,obj->hw->addr);
	//if (0 != obj->hw->addr) {
	#if ICM_AD0_LOW
		client->addr =  0xD0 >> 1;
	#else 
	    client->addr =  0xD2 >> 1;
	#endif
        GYR_INFO("gyro_use_i2c_addr: %x\n", client->addr);
   // }
	//GYR_INFO("yqf gyro_use_i2c_addr: %x\n", client->addr);
    obj_i2c_data = obj;
    obj->client = client;
    ...
    if (res) {
        GYR_ERR("icm42607_gyro_device misc register failed!\n");
	...
	res = gyro_factory_device_register(&icm42607_gyro_factory_device);
        ...
        if (res) {
                GYR_INFO("icm42607_gyro_factory_device register failed!\n");
                goto exit_misc_device_register_failed;
        }
    ...
    if (res) {
        GYR_ERR("icm42607_g create attribute err = %d\n", res);
    ...
    if (res) {
        GYR_ERR("register gyro control path err\n");
        goto exit_kfree;
    }
    ...
    if (res) {
        GYR_ERR("register gyro_data_path fail = %d\n", res);
        goto exit_kfree;
    }
    icm42607_gyro_init_flag = 0;
    GYR_INFO("%s: OK\n", __func__);
	app_get_gyro_sensor_name("icm42607");
    return 0;
...
static int icm42607_gyro_i2c_suspend(struct device *dev)
{
    ...
    struct icm42607_gyro_i2c_data *obj = i2c_get_clientdata(client);
        ...
        if (res < 0) {
            GYR_ERR("write power control fail!\n");
            return res;
        }
    
    icm42607_gyro_power(obj->hw, 0);
    GYR_INFO("icm42607_gyro suspend ok\n");
    return res;
}
...
static int icm42607_gyro_i2c_resume(struct device *dev)
{
    ...
    if (res) {
        GYR_ERR("resume client fail!!\n");
        return res;
    }
    atomic_set(&obj->suspend, 0);
    GYR_INFO("icm42607_gyro resume ok\n");
    return 0;
}

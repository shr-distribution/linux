...
static int afx133_GetOffset(void)
{
  ...
  if(g_PID == AF9133_PID || g_PID == AF8133J_PID)
  {
    ...
    coeff[0] = ((databuf[0] & 0x08) ? ((databuf[0]&0x0F) - 0x10) : (databuf[0]&0x0F));
    coeff[1] = ((databuf[0] & 0x80) ? (((databuf[0]&0xF0)>>4) - 0x10) : ((databuf[0]&0xF0)>>4));
    coeff[2] = ((databuf[1] & 0x80) ? (databuf[1] - 0x100) : databuf[1]);
    coeff[3] = ((databuf[2] & 0x80) ? (databuf[2] - 0x100) : databuf[2]);
    
    for(i=0;i<4;i++)
    {
    	int sign = (coeff[i] < 0) ? -1 : 1;
    		
      coeff[i] = sign * coeff[i];
      
      if(((i ==0 || i== 1) && coeff[i] <= 8) || 
      	 ((i ==2 || i== 3) && coeff[i] <= 30))
      {
        coeff[i] = sign * sin_table[coeff[i]];
      }
      else{
        coeff[i] = 0;	
      }	
...
static int afx133_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	printk("### %s start. ### \n",__func__);
	MAG_REST_GPIO = 435;    //GPIO92
	if (gpio_request(MAG_REST_GPIO, "mag-reset-gpio")) {
		printk("####gpio %d request failed!@@@", MAG_REST_GPIO);
		return -EPERM;
	}
	gpio_direction_output(MAG_REST_GPIO,1);
	msleep(100);

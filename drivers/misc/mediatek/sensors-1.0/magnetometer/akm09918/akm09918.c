...
#define MAG_DEBUG_ON          		0

#if MAG_DEBUG_ON
/* Log define */
#define MAG_INFO(fmt, arg...)      	printk("<<-akm09918->> "fmt"\n", ##arg)
#else
//#define GSE_TAG
#define MAG_INFO(fmt, args...)	do {} while (0)
#endif
...
static int AKECS_CheckDevice(void)
{
	...
	akm_device = buffer[1];
	MAG_INFO("akm_device = 0x%x\n",akm_device);
	if ((akm_device == 0x05) || (akm_device == 0x04)) {
		ret = AKECS_ReadFuse();
		if (ret < 0) {
			MAG_INFO("%s AKM09918: read fuse fail\n", __func__);
			return -ENXIO;
		}
					...
					int64_t maxBatchReportLatencyNs)
{
	...
	MAG_INFO("akm09918 mag set delay = (%d) ok.\n", value);
	return 0;
}
					...
					const struct i2c_device_id *id)
{
	...
	strlcpy(ctl.libinfo.libname, "akm", sizeof(ctl.libinfo.libname)); // yqf akm
	...
	app_get_m_sensor_name("akm09918");
	return 0;

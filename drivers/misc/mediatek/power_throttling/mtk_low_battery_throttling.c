...
static void dump_aging_factor(unsigned int *aging_factor_volts, unsigned int size)
{
	int i, r = 0;
	char str[128] = "";
	size_t len = sizeof(str) - 1;
...
static int check_anging_factor_order(unsigned int aging_factor) {
	int i, j, aging_factor_diff, aging_factor_volt;
	unsigned int *aging_factor_volts;
	struct lbat_thd_tbl lbat_thd_info;
	struct lbat_thd_tbl lbat_thd_info_cpy;

	lbat_thd_info_cpy.thd_volts = kmalloc(LOW_BATTERY_LEVEL_NUM * sizeof(unsigned int),
			GFP_KERNEL);
	lbat_thd_info_cpy.thd_volts_size = LOW_BATTERY_LEVEL_NUM;
	...
	for (i = 0; i <= low_bat_thl_data->temp_max_stage; i++) {
		lbat_thd_info = low_bat_thl_data->lbat_thd_info[i];
		for (j = 0; j < lbat_thd_info_cpy.thd_volts_size; j++) {
			lbat_thd_info_cpy.thd_volts[j] = lbat_thd_info.thd_volts[j];
			aging_factor_volt = aging_factor_diff * aging_factor_volts[j];
			lbat_thd_info_cpy.thd_volts[j] += aging_factor_volt;
			if (j != 0) {
				if(lbat_thd_info_cpy.thd_volts[j - 1]
					< lbat_thd_info_cpy.thd_volts[j]) {
...
static void apply_aging_factor(unsigned int aging_factor)
{
	...
	if (!low_bat_thl_data->aging_factor_enable)
		return;

	if (aging_factor == low_bat_thl_data->aging_factor)
			return;

	ret = check_anging_factor_order(aging_factor);
	if (ret != 0) {
	...
	for (i = 0; i <= low_bat_thl_data->temp_max_stage; i++) {
		for (j = 0; j < low_bat_thl_data->lbat_thd_info[i].thd_volts_size; j++) {
			aging_factor_volt = aging_factor_diff * aging_factor_volts[j];
			low_bat_thl_data->lbat_thd_info[i].thd_volts[j] += aging_factor_volt;
			low_bat_thl_data->lbat_thd_info[i].lbat_intr_info[j].volt_thd
				+= aging_factor_volt;
		}
	}
	...
	dump_thd_volts_ext(&thd_info->thd_volts[0], thd_info->thd_volts_size);
}
...
static int low_battery_thd_setting(struct platform_device *pdev, struct low_bat_thl_priv *priv)
{
	...
	if (num != (LOW_BATTERY_LEVEL_NUM -1) * (priv->temp_max_stage + 1)) {
		dev_notice(&pdev->dev, "[%s] wrong %s size %d\n", __func__, thd_volts_l, num);
		goto default_setting;
	}

	num = of_property_count_elems_of_size(np, thd_volts_h, sizeof(u32));
	if (num != (LOW_BATTERY_LEVEL_NUM - 1) * (priv->temp_max_stage + 1)) {
		dev_notice(&pdev->dev, "[%s] wrong %s size %d\n", __func__, thd_volts_h, num);
		goto default_setting;
	}
...
static int low_battery_throttling_probe(struct platform_device *pdev)
{
	...
	if (num == LOW_BATTERY_LEVEL_NUM) {
		priv->aging_factor_volts = devm_kmalloc_array(&pdev->dev, LOW_BATTERY_LEVEL_NUM,
				sizeof(u32), GFP_KERNEL);

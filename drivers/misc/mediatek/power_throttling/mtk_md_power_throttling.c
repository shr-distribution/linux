...
static int mtk_md_power_throttling_remove(struct platform_device *pdev)
{
	int i;
#if IS_ENABLED(CONFIG_MTK_LOW_BATTERY_POWER_THROTTLING)
	register_low_battery_notify(NULL, LOW_BATTERY_PRIO_MD);
#endif
#if IS_ENABLED(CONFIG_MTK_BATTERY_OC_POWER_THROTTLING)
	register_battery_oc_notify(NULL, BATTERY_OC_PRIO_MD);
#endif

	for (i = 0; i < POWER_THROTTLING_TYPE_MAX; i++) {
		kfree(md_pt_info[i].reduce_tx);
		kfree(md_pt_info[i].threshold);
	}

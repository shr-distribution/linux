...
static int mtk_cpu_power_throttling_remove(struct platform_device *pdev)
{
	...
	list_for_each_entry_safe(pt_policy, pt_policy_t, &pt_policy_list, cpu_pt_list) {
		...
		kfree(pt_policy->cpu_limit);
		kfree(pt_policy);
	}

#if IS_ENABLED(CONFIG_MTK_LOW_BATTERY_POWER_THROTTLING)
	register_low_battery_notify(NULL, LOW_BATTERY_PRIO_CPU_B);
#endif
#if IS_ENABLED(CONFIG_MTK_BATTERY_OC_POWER_THROTTLING)
	register_battery_oc_notify(NULL, BATTERY_OC_PRIO_CPU_B);
#endif
#if IS_ENABLED(CONFIG_MTK_BATTERY_PERCENT_THROTTLING)
	register_bp_thl_notify(NULL, BATTERY_PERCENT_PRIO_CPU_B);
#endif

	for (i = 0; i < POWER_THROTTLING_TYPE_MAX; i++)
		kfree(cpu_pt_info[i].freq_limit);

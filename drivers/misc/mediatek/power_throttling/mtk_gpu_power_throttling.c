...
static void gpu_pt_low_battery_cb(enum LOW_BATTERY_LEVEL_TAG level)
{
	s32 freq_limit;
...
static void gpu_pt_over_current_cb(enum BATTERY_OC_LEVEL_TAG level)
{
	s32 freq_limit;
...
static void gpu_limit_default_setting(struct device *dev, enum gpu_pt_type type)
{
	...
	if (!gpu_pt_data->freq_limit) {
		pr_notice("%s: Memory allocation failed\n", __func__);
		return;
	}

	for (i = 1; i <= gpu_pt_data->max_lv; i ++) {
		if (type == LBAT_POWER_THROTTLING && i > 1)
			gpu_pt_data->freq_limit[i-1] = GPU_LIMIT_FREQ;
		else if (type == OC_POWER_THROTTLING)
			gpu_pt_data->freq_limit[i-1] = GPU_LIMIT_FREQ;
		else
			gpu_pt_data->freq_limit[i-1] = GPUPPM_RESET_IDX;
	}
}
...
static int mtk_gpu_power_throttling_probe(struct platform_device *pdev)
{
	...
	for (i = 0; i < POWER_THROTTLING_TYPE_MAX; i++) {
		...
		gpu_pt_data->freq_limit =
                    kcalloc(gpu_pt_data->max_lv, sizeof(u32), GFP_KERNEL);
		if (!gpu_pt_data->freq_limit)
			return -ENOMEM;
	...
	if (gpu_pt_info[LBAT_POWER_THROTTLING].max_lv > 0)
		register_low_battery_notify(&gpu_pt_low_battery_cb, LOW_BATTERY_PRIO_GPU);
#endif
#if IS_ENABLED(CONFIG_MTK_BATTERY_OC_POWER_THROTTLING)
	if (gpu_pt_info[OC_POWER_THROTTLING].max_lv > 0)
		register_battery_oc_notify(&gpu_pt_over_current_cb, BATTERY_OC_PRIO_GPU);
#endif
#if IS_ENABLED(CONFIG_MTK_BATTERY_PERCENT_THROTTLING)
...
static int mtk_gpu_power_throttling_remove(struct platform_device *pdev)
{
	int i;
#if IS_ENABLED(CONFIG_MTK_LOW_BATTERY_POWER_THROTTLING)
	register_low_battery_notify(NULL, LOW_BATTERY_PRIO_GPU);
#endif
#if IS_ENABLED(CONFIG_MTK_BATTERY_OC_POWER_THROTTLING)
	register_battery_oc_notify(NULL, BATTERY_OC_PRIO_GPU);
#endif
#if IS_ENABLED(CONFIG_MTK_BATTERY_PERCENT_THROTTLING)
	register_bp_thl_notify(NULL, BATTERY_PERCENT_PRIO_GPU);
#endif
	for (i = 0; i < POWER_THROTTLING_TYPE_MAX; i++)
		kfree(gpu_pt_info[i].freq_limit);

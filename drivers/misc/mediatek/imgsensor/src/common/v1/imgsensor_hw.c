	...
	enum IMGSENSOR_HW_POWER_STATUS pwr_status)
{
	...
	if((sensor_idx != 1) && (strcmp("sp2509_mipi_raw",curr_sensor_name) == 0))  //2023.11.03
	{
		ret = IMGSENSOR_RETURN_ERROR;
		return ret;
	}
	if((sensor_idx != 0) && (strcmp("gc5025_mipi_raw",curr_sensor_name) == 0))  //2023.11.03
	{
		ret = IMGSENSOR_RETURN_ERROR;
		return ret;
	}
	if((sensor_idx != 0) && (strcmp("sp5409_mipi_raw",curr_sensor_name) == 0))  //2023.11.03
	{
		ret = IMGSENSOR_RETURN_ERROR;
		return ret;
	}
	...
	if((sensor_idx == 0) && (pwr_status == 1))
	   {
			 ...
			 atomic_set(&trigger_gc030a_thread_flag, 1);

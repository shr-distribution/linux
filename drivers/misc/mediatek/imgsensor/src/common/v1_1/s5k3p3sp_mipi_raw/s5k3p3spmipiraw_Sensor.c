...
static struct imgsensor_info_struct imgsensor_info = {
	...
	.sensor_output_dataformat = SENSOR_OUTPUT_FORMAT_RAW_Gr,  //SENSOR_OUTPUT_FORMAT_RAW_B  SENSOR_OUTPUT_FORMAT_RAW_Gr
							// SENSOR_OUTPUT_FORMAT_RAW_Gb  SENSOR_OUTPUT_FORMAT_RAW_R
	.mclk = 24,
	...
	LOG_INF("image_mirror = %d\n", image_mirror);
	 ...
	 switch (image_mirror) {
        ...
        case IMAGE_HV_MIRROR:
            write_cmos_sensor_8(0x0101,0x00);//Gb
            break;
        default:
		...
		do {
			*sensor_id = ((read_cmos_sensor_8(0x0000) << 8) | read_cmos_sensor_8(0x0001));
			...
			if (*sensor_id ==imgsensor_info.sensor_id) {
				printk("s5k3p3sp i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
			...
			printk("s5k3p3sp sensor id fail, i2c_write_id:0x%x sensor_id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
			retry--;
		} while(retry > 0);
		i++;
		retry = 2;
...
{
	printk("moshaoxi streaming_enable(0=Sw Standby,1=streaming): %d\n", enable);
	if (enable)
	{
		//write_cmos_sensor(0x0100, 0X0100);
		printk("test log111\n");
...
//    unsigned long long *feature_return_para=(unsigned long long *) feature_para;
	...
	case SENSOR_FEATURE_SET_STREAMING_SUSPEND:
		printk("moshaoxi SENSOR_FEATURE_SET_STREAMING_SUSPEND\n");
		streaming_control(KAL_FALSE);
		break;
	case SENSOR_FEATURE_SET_STREAMING_RESUME:
		printk("moshaoxi SENSOR_FEATURE_SET_STREAMING_RESUME, shutter:%llu\n",
				*feature_data);
		if (*feature_data != 0){
			set_shutter(*feature_data);
        }
...
//kin0603
//UINT32 S5K3P3SP_MIPI_RAW_SensorInit(SENSOR_FUNCTION_STRUCT **pfFunc)

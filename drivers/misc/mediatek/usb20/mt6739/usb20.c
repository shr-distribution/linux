...
{  
    int ret;
   	/* init pinctrl */
	if (usb_otg_pinctrl_init(pdev)) {
		pr_debug("Failed to init usb_otg pinctrl.\n");
		ret = -EFAULT;
		return ret;
	}
   return 0;
...
#ifdef CONFIG_USB_MTK_OTG		
static const struct of_device_id usb_otg_of_match[] = {
	{.compatible = "mediatek,mtk6739_usb_otg"},
	{},
};
MODULE_DEVICE_TABLE(of, usb_otg_of_match);

static struct platform_driver usb_otg_platform_driver = {
	.probe = usb_otg_probe,
	...
	.driver = {
		...
		.owner = THIS_MODULE,
#ifdef CONFIG_OF
		.of_match_table = usb_otg_of_match,
#endif
	},
};
#endif
...
#ifdef CONFIG_USB_MTK_OTG
   ret = platform_driver_register(&usb_otg_platform_driver);
	if (ret) {
		pr_err("Failed to register usb_otg platform driver\n");
		return ret;
	}
#endif
...
#ifdef CONFIG_USB_MTK_OTG	
	platform_driver_unregister(&usb_otg_platform_driver);
#endif
}

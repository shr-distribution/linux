...
struct pinctrl *pinctrl_otg;
struct pinctrl_state *pinctrl_drvvbus;
struct pinctrl_state *pinctrl_drvvbus_low;
struct pinctrl_state *pinctrl_drvvbus_high;

int usb_otg_pinctrl_init(struct platform_device *pdev)
{
    int ret = 0;

	pinctrl_otg = devm_pinctrl_get(&pdev->dev);
	if (IS_ERR(pinctrl_otg)) {
		dev_err(&pdev->dev, "Cannot find pinctrl_otg pinctrl!");
		ret = PTR_ERR(pinctrl_otg);
	    return ret;
	}
	
    pinctrl_drvvbus = pinctrl_lookup_state(pinctrl_otg, "drvvbus_init");
	if (IS_ERR(pinctrl_drvvbus)) {
		ret = PTR_ERR(pinctrl_drvvbus);
		dev_err(&pdev->dev, "Cannot find usb pinctrl drvvbus\n");
	    return ret;
	}

	pinctrl_drvvbus_low = pinctrl_lookup_state(pinctrl_otg, "drvvbus_low");
	...
	pinctrl_drvvbus_high = pinctrl_lookup_state(pinctrl_otg, "drvvbus_high");
	...
	pinctrl_select_state(pinctrl_otg, pinctrl_drvvbus);
...
//		charger_dev_set_boost_current_limit(primary_charger, 1500000);
	...
	pinctrl_select_state(pinctrl_otg, pinctrl_drvvbus_high);
#else
	...
	pinctrl_select_state(pinctrl_otg, pinctrl_drvvbus_low);
#else

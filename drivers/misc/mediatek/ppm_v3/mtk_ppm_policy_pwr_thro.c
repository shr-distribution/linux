...
int ppm_pwrthro_policy_init(void)
{
	int ret = 0;
...
void ppm_pwrthro_policy_exit(void)
{
	FUNC_ENTER(FUNC_LV_POLICY);
...
#ifndef DISABLE_BATTERY_PERCENT_PROTECT
	register_bp_thl_notify(NULL,
		BATTERY_PERCENT_PRIO_CPU_L);
#endif

#ifndef DISABLE_BATTERY_OC_PROTECT
	register_battery_oc_notify(NULL,
		BATTERY_OC_PRIO_CPU_L);
#endif

#ifndef DISABLE_LOW_BATTERY_PROTECT
	register_low_battery_notify(NULL,
		LOW_BATTERY_PRIO_CPU_L);
#endif

...
static ssize_t pipe_read(struct file *filp, char *buffer, ssize_t length, loff_t *offset)
{
	...
	if (copy_from_user(&mio, buffer, sizeof(mio)) != 0)
			return -EFAULT;
	...
	if (copy_to_user(buffer, &mio, sizeof(mio)) != 0)
		return -EFAULT;

	return ret;
}

static ssize_t pipe_write(struct file *filp, const char *buffer, ssize_t length, loff_t *offset)
{
	unsigned char data[4096];
	struct m_pipe *pipe = filp->private_data;
	struct modem_io mio;

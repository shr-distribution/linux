...

{
	...
	/* BBFIX: Synaptics panel on LOS23/4.19 shows a more-linear gamma vs
	 * LOS22/4.4 — measured 10x19 chart showed LOS23 midtones sit -60 to -90
	 * below LOS22 across R, G, B uniformly while blacks and whites match.
	 * Apply a uniform gamma=0.85 LUT to all three DSPP IGC channels. Since
	 * R/G/B share the same LUT, grays stay neutral — correction only affects
	 * the contrast curve, not color balance.
	 */
	if (ctl && ctl->panel_data &&
	    !strncmp(ctl->panel_data->panel_info.panel_name,
	             "Synaptics video mode dsi panel", 30)) {
		static const u32 bb_lut_contrast[256] = {
			0x000, 0x025, 0x042, 0x05e, 0x078, 0x091, 0x0a9, 0x0c1, 0x0d8, 0x0ef, 0x105, 0x11b, 0x131, 0x146, 0x15b, 0x170,
			0x185, 0x19a, 0x1ae, 0x1c2, 0x1d7, 0x1ea, 0x1fe, 0x212, 0x225, 0x239, 0x24c, 0x25f, 0x272, 0x285, 0x298, 0x2ab,
			0x2be, 0x2d0, 0x2e3, 0x2f5, 0x307, 0x31a, 0x32c, 0x33e, 0x350, 0x362, 0x374, 0x386, 0x398, 0x3a9, 0x3bb, 0x3cd,
			0x3de, 0x3f0, 0x401, 0x413, 0x424, 0x435, 0x447, 0x458, 0x469, 0x47a, 0x48b, 0x49c, 0x4ad, 0x4be, 0x4cf, 0x4e0,
			0x4f1, 0x501, 0x512, 0x523, 0x533, 0x544, 0x555, 0x565, 0x576, 0x586, 0x597, 0x5a7, 0x5b7, 0x5c8, 0x5d8, 0x5e8,
			0x5f9, 0x609, 0x619, 0x629, 0x639, 0x64a, 0x65a, 0x66a, 0x67a, 0x68a, 0x69a, 0x6aa, 0x6ba, 0x6c9, 0x6d9, 0x6e9,
			0x6f9, 0x709, 0x719, 0x728, 0x738, 0x748, 0x757, 0x767, 0x777, 0x786, 0x796, 0x7a5, 0x7b5, 0x7c4, 0x7d4, 0x7e3,
			0x7f3, 0x802, 0x812, 0x821, 0x830, 0x840, 0x84f, 0x85e, 0x86e, 0x87d, 0x88c, 0x89c, 0x8ab, 0x8ba, 0x8c9, 0x8d8,
			0x8e7, 0x8f7, 0x906, 0x915, 0x924, 0x933, 0x942, 0x951, 0x960, 0x96f, 0x97e, 0x98d, 0x99c, 0x9ab, 0x9ba, 0x9c9,
			0x9d7, 0x9e6, 0x9f5, 0xa04, 0xa13, 0xa22, 0xa30, 0xa3f, 0xa4e, 0xa5d, 0xa6b, 0xa7a, 0xa89, 0xa97, 0xaa6, 0xab5,
			0xac3, 0xad2, 0xae1, 0xaef, 0xafe, 0xb0c, 0xb1b, 0xb2a, 0xb38, 0xb47, 0xb55, 0xb64, 0xb72, 0xb81, 0xb8f, 0xb9e,
			0xbac, 0xbba, 0xbc9, 0xbd7, 0xbe6, 0xbf4, 0xc02, 0xc11, 0xc1f, 0xc2d, 0xc3c, 0xc4a, 0xc58, 0xc67, 0xc75, 0xc83,
			0xc91, 0xca0, 0xcae, 0xcbc, 0xcca, 0xcd8, 0xce7, 0xcf5, 0xd03, 0xd11, 0xd1f, 0xd2d, 0xd3c, 0xd4a, 0xd58, 0xd66,
			0xd74, 0xd82, 0xd90, 0xd9e, 0xdac, 0xdba, 0xdc8, 0xdd6, 0xde4, 0xdf2, 0xe00, 0xe0e, 0xe1c, 0xe2a, 0xe38, 0xe46,
			0xe54, 0xe62, 0xe70, 0xe7e, 0xe8b, 0xe99, 0xea7, 0xeb5, 0xec3, 0xed1, 0xedf, 0xeec, 0xefa, 0xf08, 0xf16, 0xf24,
			0xf31, 0xf3f, 0xf4d, 0xf5b, 0xf68, 0xf76, 0xf84, 0xf92, 0xf9f, 0xfad, 0xfbb, 0xfc8, 0xfd6, 0xfe4, 0xff1, 0xfff,
		};
		const u32 BB_IGC_DATA_MASK    = (1u << 12) - 1;
		const u32 BB_IGC_INDEX_UPDATE = 1u << 25;
		const u32 BB_IGC_CONFIG_MASK  =
			((((1u << (3 + 1)) - 1) & ~(1u << dspp_num)) << 28);
		const u32 BB_IGC_OP_MODE_EN   = 1u << 0;
		char __iomem *c0 = mdata->mdp_base + 0x300;
		char __iomem *c1 = c0 + 4;
		char __iomem *c2 = c1 + 4;
		u32 d0 = BB_IGC_INDEX_UPDATE | BB_IGC_CONFIG_MASK;
		u32 d  = BB_IGC_CONFIG_MASK;
		int k;

		/* Same LUT on all three channels — preserves grayscale neutrality */
		writel_relaxed((bb_lut_contrast[0] & BB_IGC_DATA_MASK) | d0, c0);
		writel_relaxed((bb_lut_contrast[0] & BB_IGC_DATA_MASK) | d0, c1);
		writel_relaxed((bb_lut_contrast[0] & BB_IGC_DATA_MASK) | d0, c2);
		for (k = 1; k < 256; k++) {
			writel_relaxed((bb_lut_contrast[k] & BB_IGC_DATA_MASK) | d, c0);
			writel_relaxed((bb_lut_contrast[k] & BB_IGC_DATA_MASK) | d, c1);
			writel_relaxed((bb_lut_contrast[k] & BB_IGC_DATA_MASK) | d, c2);
		}
		opmode |= BB_IGC_OP_MODE_EN;
		wmb();
	}

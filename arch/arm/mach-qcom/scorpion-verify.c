...
static void scorpion_dump_cpu(void *info)
{
	...
	pr_info("scorpion[cpu%d]: MIDR=0x%08x NMRR=0x%08x PRRR=0x%08x\n",
		cpu, midr, nmrr, prrr);
	pr_info("scorpion[cpu%d]: ACTLR=0x%08x (err_rep=%s mp=%s smp_nAMP=%s)\n",
		cpu, actlr,
		(actlr & 0x37) == 0x37 ? "on" : "off",
		(actlr & (1U << 24))   ? "on" : "off",
		(actlr & (1U <<  6))   ? "on" : "off");
	pr_info("scorpion[cpu%d]: L2CR0=0x%08x L2CR1=0x%08x BPCR=0x%08x\n",
		cpu, l2cr0, l2cr1, bpcr);
	pr_info("scorpion[cpu%d]: L2CPUCR=0x%08x (parity_err=%s mp_bit21=%s) SPCR=0x%08x\n",
		cpu, l2cpucr,
	...
	if (nmrr == SCORPION_NMRR_EXPECT)
		pr_info("scorpion[cpu%d]: __v7_scorpion_setup confirmed (Scorpion NMRR applied)\n",
			cpu);
	else if (nmrr == GENERIC_V7_NMRR)
		pr_warn("scorpion[cpu%d]: NMRR is generic v7 value — Scorpion proc_info NOT selected!\n",
			cpu);
	else
		pr_warn("scorpion[cpu%d]: NMRR unexpected (0x%08x) — investigate\n",
			cpu, nmrr);
}
...
static int __init scorpion_verify_init(void)
{
	...
	/*
	 * Dump CP15 state on every online CPU. CPU1 setup runs through
	 * __v7_scorpion_setup independently of CPU0 — if either CPU's
	 * MIDR doesn't catch the Scorpion proc-info entry, or if SMP
	 * bringup writes to L2CR0/L2CR1/BPCR/SPCR drift between cores,
	 * SMP-coherency bugs and IPI-from-WFI hangs can result. Have
	 * each CPU print its own state via smp_call_function_single so
	 * we can compare per-core.
	 */
	for_each_online_cpu(cpu)
		smp_call_function_single(cpu, scorpion_dump_cpu, NULL, 1);

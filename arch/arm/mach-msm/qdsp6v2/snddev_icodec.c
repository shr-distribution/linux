...
static int get_msm_cdcclk_ctl_gpios(struct platform_device *pdev)
{
    ...
    /* Palm, txmclk is tied to audio codec LDO pin, do not use */
	res = platform_get_resource_byname(pdev, IORESOURCE_IO,
			"msm_snddev_tx_mclk");
	if (!res) {
...
#if defined(CONFIG_MARIMBA_CODEC)
    /* Palm: we use txmclk pin (GPIO108) as LDO to power audio codec,
     * Turning on clk will disable the LDO, only do this if using marimba */

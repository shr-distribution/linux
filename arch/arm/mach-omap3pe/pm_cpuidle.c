...
static struct omap3_processor_cx omap3_power_states[OMAP3_MAX_STATES] = {
	...
	{ /* C7 */
		...
		10000,
		30000,
		300000,		/* FIXME: can be lowered with optimization */
		PRCM_MPU_OFF,
		PRCM_CORE_OFF,
...
static void omap3_idle_setup_wkup_sources (void)
{
	...
	/* Revisit-Hack: Clear out pending PRCM interrupts. The PRCM interrupt
	 * handler clear at wakeup time with out any tracking. The handler
	 * won't be called for a SR or VC interrupt.  Status left about seems to
	 * cause some issues with SR operation.
	 */
			...
			struct cpuidle_state *state)
{
	...
	if (target_state.core_state > PRCM_CORE_ACTIVE) {
		...
		core_sleep_flg = 1;
		toggle_smartreflex = (omap2_disp_lpr_is_enabled() &&
				CM_FCLKEN_DSS & DSS_FCLK_MASK);
		...
		/* If LPR is enabled don't shut down SR as there will be not scaling */
		if(toggle_smartreflex){

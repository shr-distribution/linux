...
static u8 core_l3_vdd2_volts [2][PRCM_NO_VDD2_OPPS] = { /* only 3 OPPs */
	/* Vsel corresponding to 0.975V (OPP1), 1.05V (OPP2), 1.15V (OPP3) */
	{ 0x1e, 0x24, 0x2c },    /* OLD 3430 values */
	/* Vsel corresponding to unused (OPP1), 0.9375 (OPP2), 1.1375V (OPP3) */
	{ 0x14, 0x1b, 0x2b },    /* NEW 3630 values */
};

u8 get_mpu_iva2_vdd1_volts(int opp_num)
{
	return mpu_iva2_vdd1_volts[tidx][opp_num-1];
}

u8 get_core_l3_vdd2_volts(int opp_num)
{
	return core_l3_vdd2_volts [tidx][opp_num-1];
}

static struct dvfs_config omap3_vdd2_config[2][2] = {
	{
	{
	/* SDRC CS0/CS1 values 83MHZ*/
	{{0x00025501, 0x629db4c6, 0x00012214},
	 {0x00025501, 0x629db4c6, 0x00012214} },
	...
	{
	{{0x0004e201, 0xbaa1c4c6, 0x00022122},
	 {0x0004e201, 0xbaa1c4c6, 0x00022122} },
	...
	{
	{
	/* SDRC CS0/CS1 values 100MHZ*/
	{{0x00025501, 0xc2e1c4c6, 0x00022218},
	 {0x00025501, 0xc2e1c4c6, 0x00022218} },
	/* GPMC CS0/1/2/3 values 200 MHZ*/
	/* CS0 values are for NOR */
	/* CS1 values are for NAND */
	/* CS2 values are for ONE-NAND */
	/* CS3 values are for FPGA */
...
// -wgr- 	{
// -wgr- 	{
// -wgr- 	/* SDRC CS0/CS1 values 100MHZ*/
// -wgr- 	{{0x00025501, 0x629db4c6, 0x00022218},
// -wgr- 	 {0x00025501, 0x629db4c6, 0x00022218} },
// -wgr- 	/* GPMC CS0/1/2/3 values 200 MHZ*/
// -wgr- 	/* CS0 values are for NOR */
// -wgr- 	/* CS1 values are for NAND */
// -wgr- 	/* CS2 values are for ONE-NAND */
// -wgr- 	/* CS3 values are for FPGA */
// -wgr- 	{{0x00101000, 0x00040401, 0x0E050E05, 0x010A1010, 0x100802C1},
// -wgr- 	 {0x000A0A00, 0x000A0A00, 0x08010801, 0x01060A0A, 0x10080580},
// -wgr- 	 {0x00080801, 0x00020201, 0x08020802, 0x01080808, 0x10030000},
// -wgr- 	 {0x00101001, 0x00040402, 0x0E050E05, 0x020F1010, 0x0F0502C2} }
// -wgr- 	},
// -wgr-
// -wgr- 	/* SDRC CS0/CS1 values 200MHZ*/
// -wgr- 	{
// -wgr- 	{{0x0004e201, 0x629db4c6, 0x00012214},
// -wgr- 	 {0x0004e201, 0x629db4c6, 0x00012214} },
// -wgr- 	/* GPMC CS0/1/2/3 values 200 MHZ*/
// -wgr- 	{{0x001f1f00, 0x00080802, 0x1C091C09, 0x01131F1F, 0x1F0F03C2},
// -wgr- 	 {0x00141400, 0x00141400, 0x0F010F01, 0x010C1414, 0x1F0F0A80},
// -wgr- 	 {0x000F0F01, 0x00030301, 0x0F040F04, 0x010F1010, 0x1F060000},
// -wgr- 	 {0x001F1F01, 0x00080803, 0x1D091D09, 0x041D1F1F, 0x1D0904C4} }
// -wgr- 	},
// -wgr- 	},
};
...
static u32 s_current_vdd1_opp = 0;
static u32 s_current_vdd2_opp = 0;
...
{
	return s_current_vdd1_opp;
}

u32 prcm_get_current_vdd2_opp(void)
{
	return s_current_vdd2_opp;
}

void prcm_set_current_vdd1_opp(u32 opp)
{
	s_current_vdd1_opp = opp;
}

void prcm_set_current_vdd2_opp(u32 opp)
{
	s_current_vdd2_opp = opp;
}
...
/* MPU parameters */
static struct dpll_param mpu_dpll_param[2][5][PRCM_NO_VDD1_OPPS] = {
	{ /* 3430 settings */
	/* 12M values */
	...
	{ /* 3630 settings */
	/********* Only 13M and 26M values are supported! *******/
	/* 12M values */
	/* OPP1(125 Mhz) and OPP2(250 Mhz)*/
	{{0x0FA, 0x05, 0x07, 0x04}, {0x0FA, 0x05, 0x07, 0x02},
	/* OPP3(500 Mhz) and OPP4(550 Mhz)*/
...
/* IVA parameters */
static struct dpll_param iva_dpll_param[2][5][PRCM_NO_VDD1_OPPS] = {
	{ /* 3430 settings */
	/* 12M values */
...
/* CORE parameters */
static struct dpll_param core_dpll_param[2][5][PRCM_NO_VDD2_OPPS] = {
	{ /* 3430 settings */
	/* 12M values */
	...
	/* 12M values */
	/* OPP1(100 Mhz) and OPP2(200 Mhz) */
	{{0, 0, 0, 0}, {0x0A6, 0x05, 0x07, 0x02}, {0x0A6, 0x05, 0x07, 0x01} },
	/* 13M values */
	/* OPP1(100 Mhz) and OPP2(200 Mhz) */
	{{0, 0, 0, 0}, {0x190, 0x0C, 0x03, 0x02}, {0x190, 0x0C, 0x03, 0x01} },
	/* 19.2M values */
	/* OPP1(100 Mhz) and OPP2(200 Mhz) */
	{{0, 0, 0, 0}, {0x19F, 0x17, 0x03, 0x02}, {0x19F, 0x17, 0x03, 0x01} },
	/* 26M values */
	/* OPP1(100 Mhz) and OPP2(200 Mhz) */
	{{0, 0, 0, 0}, {0x0C8, 0x0C, 0x07, 0x02}, {0x0C8, 0x0C, 0x07, 0x01} },
	/* 38.4M values */
	/* OPP1(100 Mhz) and OPP2(200 Mhz) */
	{{0, 0, 0, 0}, {0x19F, 0x2F, 0x03, 0x02}, {0x19F, 0x2F, 0x03, 0x01} },
	},
...
#if defined(CONFIG_OMAP_VOLT_VSEL)
	if (vdd == PRCM_VDD1)
		vsel = mpu_iva2_vdd1_volts[tidx][target_opp_no - 1];

	else if (vdd == PRCM_VDD2)
		vsel = core_l3_vdd2_volts[tidx][target_opp_no - 1];
...
#else
	/* 
	 * sr_voltagescale must be called with last argument set to 0 
	 * in this case it will automatically pickup voltage apropriate
	 * for requested operating point.
	 */
	if (sr_voltagescale(target_opp_id, current_opp_id, 0)) {
		printk(KERN_ERR "Unable to set the voltage level\n");
		return PRCM_FAIL;
	}
...
void prcm_scale_prepare(void)
{
	...
	if (get_current_vdd1_opp() != PRCM_VDD1_OPP1) {
#ifdef CONFIG_MACH_SIRLOIN_3630
		valid_rate = clk_round_rate(p_vdd1_clk, S150M);
#else
		valid_rate = clk_round_rate(p_vdd1_clk, S125M);
#endif
		p_vdd1_clk->set_rate(p_vdd1_clk, valid_rate);
		prcm_do_voltage_scaling(PRCM_VDD1_OPP1, get_current_vdd1_opp());
	}
	if (get_current_vdd2_opp() != PRCM_VDD2_OPP2) {
#ifdef CONFIG_MACH_SIRLOIN_3630
		valid_rate = clk_round_rate(p_vdd2_clk, S100M);
#else
		valid_rate = clk_round_rate(p_vdd2_clk, S83M);
#endif
		p_vdd1_clk->set_rate(p_vdd2_clk, valid_rate);
		prcm_do_voltage_scaling(PRCM_VDD2_OPP2, get_current_vdd2_opp());
	}
}
...
void prcm_scale_finish(void)
{
	...
	prcm_do_voltage_scaling(get_current_vdd1_opp(), PRCM_VDD1_OPP1);
	if (get_current_vdd1_opp != PRCM_VDD1_OPP1) {
		switch (get_current_vdd1_opp()) {
#ifdef CONFIG_MACH_SIRLOIN_3630
		case PRCM_VDD1_OPP2:
...
static struct vdd2_core_freq_d {
	...
	{100, CO_VDD2_OPP2, PRCM_VDD2_OPP2},
...
{
	u32 valid_rate;
	u32 cur_opp_no, target_vdd1_opp, cur_vdd1_opp;
	

	target_vdd1_opp = vdd1_arm_dsp_freq[target_opp_no-1].opp;
	cur_vdd1_opp    = prcm_get_current_vdd1_opp();
	cur_opp_no      = get_opp_no(cur_vdd1_opp);
	...
	if (cur_opp_no < target_opp_no) {
		prcm_do_voltage_scaling(target_vdd1_opp, cur_vdd1_opp);
...
{
	u32 valid_rate;
	u32 cur_opp_no, target_vdd2_opp, cur_vdd2_opp;

	target_vdd2_opp = vdd2_core_freq[target_opp_no-1].opp;
	cur_vdd2_opp    = prcm_get_current_vdd2_opp();
	cur_opp_no      = get_opp_no(cur_vdd2_opp);

	if (p_vdd2_clk == NULL)
		p_vdd2_clk = clk_get(NULL, "virt_vdd2_prcm_set");

	if (cur_opp_no < target_opp_no) {
		prcm_do_voltage_scaling(target_vdd2_opp, cur_vdd2_opp);
...
unsigned int get_arm_freq_for_opp(unsigned int opp)
{
	...
	if (!opp || opp > ARRAY_SIZE(vdd1_arm_dsp_freq)) {
		panic("MPU frequencey request for illegal OPP %d\n", opp);
		return 0;
	}
...
unsigned int get_dsp_freq_for_opp(unsigned int opp)
{
	/* NOTE: opp index is 1 based */
	if (!opp || opp > ARRAY_SIZE(vdd1_arm_dsp_freq)) {
		panic("ARM frequencey request for illegal OPP %d\n", opp);
		return 0;
	}
...
unsigned int get_opp_for_index(unsigned int idx)
{
	if (idx >= ARRAY_SIZE(vdd1_arm_dsp_freq)) {
		panic("OPP request for illegal index %d\n", idx);
		return 0;
	}
 ...
 */
static ssize_t 
omap_pm_vdd1_opp_show(struct kset *subsys, char *buf)
{
	return sprintf(buf, "%x\n", get_opp_no(prcm_get_current_vdd1_opp()));
}

static ssize_t 
omap_pm_vdd1_opp_store(struct kset *subsys, const char *buf, size_t n)
{
	u32 target_opp_no;

	target_opp_no = (u32) simple_strtoul ( buf, NULL, 0 );

	/* Check target OPP for valid range. */
	if ((target_opp_no < MIN_VDD1_OPP) || (target_opp_no > MAX_VDD1_OPP )) {
		printk(KERN_ERR "vdd1_opp_store: invalid opp number (%d)\n",
		       target_opp_no);
		return -EINVAL;
	}

	/* We would like to ignore opp1 as there is no benefit for using it */
	if (target_opp_no == CO_VDD1_OPP1) {
		target_opp_no = CO_VDD1_OPP2;
	}
 ...
 */
static ssize_t 
omap_pm_vdd2_opp_show(struct kset *subsys, char *buf)
{
	return sprintf(buf, "%x\n", get_opp_no(prcm_get_current_vdd2_opp()));
}

static ssize_t 
omap_pm_vdd2_opp_store(struct kset *subsys, const char *buf, size_t n)
{
	u32 target_opp_no;

	target_opp_no = (u32) simple_strtoul ( buf, NULL, 0 );
	
	if ((target_opp_no < MIN_VDD2_OPP) || (target_opp_no > MAX_VDD2_OPP)) {
		printk(KERN_ERR "vdd2_opp_store: invalid opp number (%d)\n",
		       target_opp_no );
		return -EINVAL;
	}
...
static __init int omap_setup_pm_fwk_constraints(void)
{
	if (vdd1_opp_co == NULL)
		vdd1_opp_co = constraint_get("pm_fwk", &cnstr_id1);

	constraint_set(vdd1_opp_co, CO_VDD1_OPP2);

	if (vdd2_opp_co == NULL)
		vdd2_opp_co = constraint_get("pm_fwk", &cnstr_id2);
	constraint_set(vdd2_opp_co, CO_VDD2_OPP2);
	
	return 0;
}
late_initcall_sync(omap_setup_pm_fwk_constraints);

#ifdef SDRC_DEBUG
/*
 *
 */
static ssize_t omap_sdrc_regs_show(struct kset *subsys, char *buf)
{
	int l = 0;

	l += sprintf(buf+l, "SDRC_DLLA_CTRL     = 0x%08x\n", SDRC_DLLA_CTRL);
	l += sprintf(buf+l, "SDRC_DLLA_STATUS   = 0x%08x\n", SDRC_DLLA_STATUS);
	l += sprintf(buf+l, "SDRC_RFR_CTRL_0    = 0x%08x\n", SDRC_RFR_CTRL_0);
	l += sprintf(buf+l, "SDRC_ACTIM_CTRLA_0 = 0x%08x\n", SDRC_ACTIM_CTRLA_0);
	l += sprintf(buf+l, "SDRC_ACTIM_CTRLB_0 = 0x%08x\n", SDRC_ACTIM_CTRLB_0);
	l += sprintf(buf+l, "SDRC_RFR_CTRL_1    = 0x%08x\n", SDRC_RFR_CTRL_1);
	l += sprintf(buf+l, "SDRC_ACTIM_CTRLA_1 = 0x%08x\n", SDRC_ACTIM_CTRLA_1);
	l += sprintf(buf+l, "SDRC_ACTIM_CTRLB_1 = 0x%08x\n", SDRC_ACTIM_CTRLB_1);
	l += sprintf(buf+l, "CM_CLKSEL1_PLL     = 0x%08x\n", CM_CLKSEL1_PLL);
	l += sprintf(buf+l, "CM_CLKSEL_CORE     = 0x%08x\n", CM_CLKSEL_CORE);
	
	return l;
}

static struct subsys_attribute sdrc_regs_attr = {
	.attr = {
		.name = __stringify(sdrc_regs),
		.mode = 0644,
	},
	.show = omap_sdrc_regs_show,
};
#endif
		...
		*mn_output = ((sys_clkspeed * mult) / (div + 1));
		...
		if ((dpll_id == DPLL3_CORE) ||
			...
			*mn_output = sys_clkspeed;
			...
			bypassclk_divider = (clksel1_pll >> 19) & 0x7;
...
int prcm_lock_iva_dpll(u32 target_opp_id)
{
	...
	tar_m = iva_dpll_param[tidx][index][target_opp_no - 1].dpll_m;
	tar_n = iva_dpll_param[tidx][index][target_opp_no - 1].dpll_n;
	...
	tar_freqsel = iva_dpll_param[tidx][index][target_opp_no - 1].dpll_freqsel;
...
static int vdd2_lockdelay[CO_VDD1_MAXLEVEL] = {
	16, 
	 8,  /* opp1 */
	12,  /* opp2 */
	16,  /* opp3 */
...
int prcm_do_frequency_scaling(u32 target_opp_id, u32 current_opp_id)
{
	...
	if (vdd == PRCM_VDD1) {
		curr_opp_no = current_opp_id & OPP_NO_MASK;
		target_opp_no = target_opp_id & OPP_NO_MASK;
		curr_m = (mpu_dpll_param[tidx][index][curr_opp_no - 1].dpll_m);
		curr_n = (mpu_dpll_param[tidx][index][curr_opp_no - 1].dpll_n);
		tar_m = (mpu_dpll_param[tidx][index][target_opp_no - 1].dpll_m);
		tar_n = (mpu_dpll_param[tidx][index][target_opp_no - 1].dpll_n);
		...
		if ((curr_m != tar_m) || (curr_n != tar_n)) {
			...
			/* Set M,N,Freqsel values */
			tar_freqsel =
				mpu_dpll_param[tidx][index][target_opp_no - 1].dpll_freqsel;
			prcm_configure_dpll(DPLL1_MPU, tar_m, tar_n, tar_freqsel);
			/* Lock the DPLL */
			prcm_enable_dpll(DPLL1_MPU);
		}
		/* Configure M2 */
		tar_m2 = mpu_dpll_param[tidx][index][target_opp_no - 1].dpll_m2;
		prcm_configure_dpll_divider(PRCM_DPLL1_M2X2_CLK, tar_m2);

		curr_m = iva_dpll_param[tidx][index][curr_opp_no - 1].dpll_m;
		curr_n = iva_dpll_param[tidx][index][curr_opp_no - 1].dpll_n;
		tar_m =  iva_dpll_param[tidx][index][target_opp_no - 1].dpll_m;
		tar_n =  iva_dpll_param[tidx][index][target_opp_no - 1].dpll_n;
		...
		if ((curr_m != tar_m) || (curr_n != tar_n)) {
			...
			/* Set M,N,Freqsel values */
			tar_freqsel =
				iva_dpll_param[tidx][index][target_opp_no - 1].dpll_freqsel;
			prcm_configure_dpll(DPLL2_IVA2, tar_m, tar_n, tar_freqsel);
			/* Lock the DPLL */
			prcm_enable_dpll(DPLL2_IVA2);
		}
		/* Configure M2 */
		tar_m2 = iva_dpll_param[tidx][index][target_opp_no - 1].dpll_m2;
...
//		 printk("VDD1 => OPP%d\n", target_opp_no);
		...
		u32 unlock_dll, inc, lockdelay, vdd1_opp_no;
		
		target_opp_no = target_opp_id & OPP_NO_MASK;
		tar_m = (core_dpll_param[tidx][index][target_opp_no - 1].dpll_m);
		tar_n = (core_dpll_param[tidx][index][target_opp_no - 1].dpll_n);
		tar_freqsel = (core_dpll_param[tidx][index]
					[target_opp_no - 1].dpll_freqsel);
		tar_m2 = (core_dpll_param[tidx][index][target_opp_no - 1].dpll_m2);
		/* This function is executed from SRAM */
		local_irq_save(flags);
		vdd1_opp_no = get_opp_no(prcm_get_current_vdd1_opp());

		if (target_opp_id == PRCM_VDD2_OPP3) {
			omap3_configure_gpmc(&omap3_vdd2_config[tidx][1]);
			inc = 1;
			lockdelay   = vdd2_lockdelay[vdd1_opp_no];
			unlock_dll  = 0;
			rfr_ctrl    = omap3_vdd2_config[tidx][1].sdrc_cfg[0].sdrc_rfr_ctrl;
			actim_ctrla = omap3_vdd2_config[tidx][1].sdrc_cfg[0].sdrc_actim_ctrla;
			actim_ctrlb = omap3_vdd2_config[tidx][1].sdrc_cfg[0].sdrc_actim_ctrlb;
//			printk("VDD2 => OPP3\n");
		} else if (target_opp_id == PRCM_VDD2_OPP2) {
			inc = 0;
			lockdelay   = vdd2_lockdelay[vdd1_opp_no];
			unlock_dll  = 0;
			rfr_ctrl    = omap3_vdd2_config[tidx][0].sdrc_cfg[0].sdrc_rfr_ctrl;
			actim_ctrla = omap3_vdd2_config[tidx][0].sdrc_cfg[0].sdrc_actim_ctrla;
			actim_ctrlb = omap3_vdd2_config[tidx][0].sdrc_cfg[0].sdrc_actim_ctrlb;
//			printk("VDD2 => OPP2\n");
		} else {
			panic("############%s: opp?? (0x%x)\n", __func__, target_opp_id );
		}

		(void) omap3_configure_core_dpll(
			tar_m2, unlock_dll, lockdelay, inc,
			rfr_ctrl, actim_ctrla, actim_ctrlb, 0,
			rfr_ctrl, actim_ctrla, actim_ctrlb, 0);
...
extern void omap2_disp_pre_freq_change (void);
extern void omap2_disp_post_freq_change(void);

/*
 * Make resource framework in sync with reality
 */
int __init prcm_fixup_resource_framework (void)
{
	struct res_handle *res = NULL;
	u32 real_vdd1_opp_no, real_vdd2_opp_no;

	real_vdd1_opp_no = get_opp_no(prcm_get_current_vdd1_opp());
	real_vdd2_opp_no = get_opp_no(prcm_get_current_vdd2_opp());


	res = resource_get("prcm_opp", "vdd1_opp");
	BUG_ON(!res);
	resource_fixup_level(res, real_vdd1_opp_no );
	resource_put(res);

	res = resource_get("prcm_opp", "vdd2_opp");
	BUG_ON(!res);
	resource_fixup_level(res, real_vdd2_opp_no );
	resource_put(res);

	res = resource_get("prcm_opp", "arm_freq");
	BUG_ON(!res);
	resource_fixup_level(res,
		get_arm_freq_for_opp(real_vdd1_opp_no)/1000);
	resource_put(res);

	res = resource_get("prcm_opp", "dsp_freq");
	BUG_ON(!res);
	resource_fixup_level(res,
		get_dsp_freq_for_opp(real_vdd1_opp_no)/1000);
	resource_put(res);
	return 0;
}
...
{
	u32 mpu_khz, target_mpu_khz;
	u32 target_vdd1_opp, target_vdd2_opp;
...
#if   defined(CONFIG_OMAP3_VDD1_OPP1)
	target_vdd1_opp = PRCM_VDD1_OPP1;
#elif defined(CONFIG_OMAP3_VDD1_OPP2)
	target_vdd1_opp = PRCM_VDD1_OPP2;
#elif defined(CONFIG_OMAP3_VDD1_OPP3)
	target_vdd1_opp = PRCM_VDD1_OPP3;
#elif defined(CONFIG_OMAP3_VDD1_OPP4)
...
#error  "Undefined default VDD1 operation point"
	...
	/*  Setup VDD2 */
#if   defined(CONFIG_OMAP3_VDD2_OPP2)
	target_vdd2_opp = PRCM_VDD2_OPP2;
#elif defined(CONFIG_OMAP3_VDD2_OPP3)
	target_vdd2_opp = PRCM_VDD2_OPP3;
#else	
#error  "Undefined default VDD2 operation point"
#endif

	mpu_khz = rnd_rate_vdd2[get_opp_no(prcm_get_current_vdd2_opp())-1];
	target_mpu_khz = rnd_rate_vdd2[get_opp_no(target_vdd2_opp)-1];
	printk(KERN_INFO "OMAP: Change CORE speed: %d => %d MHz\n",
	       mpu_khz/1000000, target_mpu_khz/1000000);

	/* 
	 * Changing vdd2 opp causes display glitch. 
	 * This sequence works around that but it is a bit of a hack.
	 */
	omap2_disp_pre_freq_change ();
	vdd2_opp_setting(get_opp_no(target_vdd2_opp));
	omap2_disp_post_freq_change();

	prcm_fixup_resource_framework ();
...
#ifdef SDRC_DEBUG
	if (0 != subsys_create_file(&power_subsys, &sdrc_regs_attr)) {
		printk(KERN_ERR "Could not create sysfs entry for SDRC.\n");
	}
#endif
#endif

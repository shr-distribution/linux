...
void save_scratchpad_contents(void)
{
	...
	if( cpu_is_omap36xx()) {
		...
		if( is_omap3_rev_equal_to(OMAP3630_REV_ES1_1))
			restore_address = get_3630_es1_1_restore_pointer();
		else
			restore_address = get_3430_es3_restore_pointer();
	} else {
	...
	*(scratchpad_address++) = CM_CLKEN_PLL;
	/* 
	 *   It was found that if CM_AUTOIDLE_PLL.AUTO_PERIPH_DPLL is stored 
	 *   as 1 in the scratchpad then the wake up latency from OFF mode 
	 *   increased to 23msec.
	 *   
	 *   Reducing wakeup latency is a good thing by itself, but it is 
	 *   really important for system with 3630 ES1.1 for tXSR violation 
	 *   software workaround to function properly (Errata 1.90). 
	 *
	 *   The masked value will be loaded into CM_AUTOIDLE_PLL register by ROM 
	 *   during WU from OFF mode transition. It is not nessesary to explicitly restore
	 *   original value, it will be done as a part of PRCM_SAVE/RESTORE
	 *   operation.
	 */
	*(scratchpad_address++) = CM_AUTOIDLE_PLL & 0x7;
	...
	*(scratchpad_address++) = 0x0;
	...
	if( cpu_is_omap36xx()) {
		...
		if( is_omap3_rev_equal_to(OMAP3630_REV_ES1_1)) {
			/* Errata 1.90. Self_Refresh Exit issue after OFF mode */
			v &= ~(SDRC_PWR_AUTOCOUNT_MASK | SDRC_PWR_CLKCTRL_MASK);
		}
		*(scratchpad_address++) = v;
	}
	else
#endif	
	/* Errata 1.142:
	 ...
	 */
	if (!is_device_type_gp() && is_sil_rev_greater_than(OMAP3430_REV_ES2_1)) {
		u32 v = SDRC_POWER_REG;
		v &= ~(SDRC_PWR_AUTOCOUNT_MASK | SDRC_PWR_CLKCTRL_MASK);
		v |= 120;
		*(scratchpad_address++) = v;

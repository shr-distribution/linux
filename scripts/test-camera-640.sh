...
setup_media_pipeline() {
    ...
    # - Context B: 640x480 (capture/full resolution)
...
# MT9M113 supported: 640x480 (preview), 640x480 (capture)
# Using 640x480 for full capture mode testing
echo ''
echo 'Setting sensor format (640x480 UYVY8_2X8)...'
media-ctl -d \$MEDIA_DEV -V \"'\$SENSOR':0[fmt:UYVY8_2X8/640x480]\" 2>&1 || \
media-ctl -d \$MEDIA_DEV -V \"'\$SENSOR':0[fmt:UYVY8_1X16/640x480]\" 2>&1 || \
echo 'Sensor format set may have failed'

# Set CSIPHY format (sink and source)
echo 'Setting CSIPHY format...'
media-ctl -d \$MEDIA_DEV -V \"'\$CSIPHY':0[fmt:UYVY8_2X8/640x480]\" 2>&1 || true
media-ctl -d \$MEDIA_DEV -V \"'\$CSIPHY':1[fmt:UYVY8_2X8/640x480]\" 2>&1 || true

# Set CSID format (sink=0, PIX source=4)
echo 'Setting CSID format...'
media-ctl -d \$MEDIA_DEV -V \"'\$CSID':0[fmt:UYVY8_2X8/640x480]\" 2>&1 || true
media-ctl -d \$MEDIA_DEV -V \"'\$CSID':4[fmt:UYVY8_2X8/640x480]\" 2>&1 || true

# Set VFE PIX format
echo 'Setting VFE format...'
if [ -n \"\$VFE_PIX\" ]; then
    media-ctl -d \$MEDIA_DEV -V \"'\$VFE_PIX':0[fmt:UYVY8_2X8/640x480]\" 2>&1 || true
else
    media-ctl -d \$MEDIA_DEV -V \"'\$VFE':0[fmt:UYVY8_2X8/640x480]\" 2>&1 || true
fi
...
if command -v v4l2-ctl >/dev/null 2>&1; then
    ...
    # MT9M113 supported: 640x480 (preview), 640x480 (capture)
    echo 'Setting video format to 640x480 UYVY (MT9M113 capture mode)...'
    v4l2-ctl -d \$VIDEO_DEV --set-fmt-video=width=640,height=480,pixelformat=UYVY 2>&1
...
if command -v gst-launch-1.0 >/dev/null 2>&1; then
    ...
    # MT9M113 supported: 640x480 (preview), 640x480 (capture)
    echo 'Testing pipeline with fakesink (640x480 UYVY)...'
...
quick_capture_test() {
    ...
    run_on_device "
        if command -v gst-launch-1.0 >/dev/null 2>&1; then
            # MT9M113 supported: 640x480 (preview), 640x480 (capture)
            echo '=== Test: 640x480 UYVY (MT9M113 capture mode) ==='
...
test_raw_mode() {
    ...
    log_info "Using 640x480 (Context B - full resolution)"
    ...
    run_on_device "
        ...
        # Set formats on the RDI path (640x480 = MT9M113 Context B full capture)
        # IMPORTANT: Set compose rectangle on IFP pad 0 to trigger Context B
        echo 'Setting formats (640x480 Context B capture mode)...'
        media-ctl -V '\"\$SENSOR\":0[compose:(0,0)/640x480]' 2>&1 || true
        ...
        media-ctl -V '\"msm_csid1\":4[fmt:UYVY8_1X16/640x480]' 2>&1 || true
        ...
        if [ \$? -eq 0 ]; then
            ...
            if [ -s /tmp/camera_raw.raw ]; then
                ...
                echo \"Frame saved: \$SIZE bytes (expected 1966080 for 640x480 UYVY)\"
            fi
        else
            echo ''
...
test_testgen_mode() {
    ...
    run_on_device "
        ...
        # Set formats on entire pipeline (640x480)
        echo 'Setting formats (640x480 UYVY8_2X8)...'
        media-ctl -V '\"\$SENSOR_BASE\":0[fmt:UYVY8_2X8/640x480]' 2>&1 || true
...
test_pix_mode() {
    ...
    run_on_device "
        ...
        # Set formats on entire pipeline (640x480 = MT9M113 Context B full capture)
        # IMPORTANT: Set compose rectangle on IFP pad 0 to trigger Context B
        echo 'Setting formats (640x480 Context B capture mode)...'
        media-ctl -V '\"\$SENSOR\":0[compose:(0,0)/640x480]' 2>&1 || true
        media-ctl -V '\"\$SENSOR\":1[fmt:UYVY8_1X16/640x480]' 2>&1 || true
        ...
        echo ''
        echo 'Testing capture with 640x480 NV16 (MT9M113 Context B)...'
        ...
        if [ \$? -eq 0 ]; then
            ...
            if [ -s /tmp/camera_pix.raw ]; then
                ...
                echo \"Frame saved: \$SIZE bytes (expected 1966080 for 640x480 NV12)\"
            fi
        else
            echo ''
...
test_v4l2_mode() {
    ...
    run_on_device "
        ...
        # Set formats on entire pipeline (640x480 = MT9M113 Context B)
        # IMPORTANT: Set compose rectangle on IFP pad 0 to trigger Context B
        # The sensor output format on pad 1 is derived from the compose size
        echo 'Setting formats (640x480)...'
        echo 'Setting compose rectangle to 640x480 (triggers Context B)...'
        media-ctl -V '\"\$SENSOR\":0[compose:(0,0)/640x480]' 2>&1 || true
        media-ctl -V '\"\$SENSOR\":1[fmt:UYVY8_1X16/640x480]' 2>&1 || true
        media-ctl -V '\"msm_csiphy1\":0[fmt:UYVY8_1X16/640x480]' 2>&1 || true
        media-ctl -V '\"msm_csiphy1\":1[fmt:UYVY8_1X16/640x480]' 2>&1 || true
...
test_video_mode() {
    ...
    run_on_device "
        ...
        echo ''
        echo 'Testing capture with 640x480 NV16 (VIDEO mode)...'
        timeout 20 gst-launch-1.0 -v v4l2src device=/dev/video3 num-buffers=10 ! \\
            'video/x-raw,format=NV12,width=640,height=480,framerate=30/1' ! \\
            fakesink 2>&1
...
test_video4_mode() {
    ...
    run_on_device "
        ...
        echo 'Using FULL RESOLUTION 640x480 (MT9M113 Context B capture mode)'
        echo ''
        ...
        # IMPORTANT: Set compose rectangle on IFP pad 0 to trigger Context B
        # The sensor output format on pad 1 is derived from the compose size
        echo 'Setting formats (640x480 capture mode)...'
        echo 'Setting compose rectangle to 640x480 (triggers Context B)...'
        media-ctl -V '\"\$SENSOR\":0[compose:(0,0)/640x480]' 2>&1 || true
        media-ctl -V '\"\$SENSOR\":1[fmt:UYVY8_1X16/640x480]' 2>&1 || true
        media-ctl -V '\"msm_csiphy1\":0[fmt:UYVY8_1X16/640x480]' 2>&1 || true
        ...
        media-ctl -V '\"msm_csid1\":0[fmt:UYVY8_1X16/640x480]' 2>&1 || true
        media-ctl -V '\"msm_csid1\":4[fmt:UYVY8_2X8/640x480]' 2>&1 || true
        media-ctl -V '\"msm_vfe0_video\":0[fmt:UYVY8_2X8/640x480]' 2>&1 || true
        media-ctl -V '\"msm_vfe0_video\":1[fmt:UYVY8_2X8/640x480]' 2>&1 || true

        # Set V4L2 video device format to NV12 (native VFE31 output)
        echo ''
        echo 'Setting V4L2 format to NV16 640x480 on /dev/video4...'
        ...
        echo ''
        echo 'Testing capture with 640x480 NV16 on VIDEO4...'
...
test_nv16_mode() {
    ...
    run_on_device "
        ...
        echo ''
        echo 'Setting V4L2 format to NV16 640x480...'
        v4l2-ctl -d /dev/video3 --set-fmt-video=width=640,height=480,pixelformat=NV12 2>&1 || true
        echo ''
        echo 'Current format:'
        v4l2-ctl -d /dev/video3 --get-fmt-video 2>&1
...
capture_frames() {
    ...
    run_on_device "
        ...
        # Set formats - set compose first to trigger Context B for 640x480
...
test_legacy_mode() {
    ...
    run_on_device "
        ...
        echo ''
        echo 'Testing capture with 640x480 UYVY (LEGACY mode)...'
        timeout 15 gst-launch-1.0 -v v4l2src device=/dev/video3 num-buffers=10 ! \\
            'video/x-raw,format=UYVY,width=640,height=480,framerate=30/1' ! \\
            fakesink 2>&1
...
test_at_resolution() {
    ...
    run_on_device "
        ...
        media-ctl -d /dev/media0 -V \"\\\"$csid\\\":0[fmt:\${PIPE_FMT}/${width}x${height}]\" 2>&1 || true
        media-ctl -d /dev/media0 -V \"\\\"$csid\\\":\${CSID_PAD}[fmt:\${PIPE_FMT}/${width}x${height}]\" 2>&1 || true
...
test_comprehensive() {
    ...
    echo "  Resolutions: 640x480, 640x480"
...
main() {
    ...
    for arg in "$@"; do
        case $arg in
            ...
            --help|-h)
                ...
                echo "  comprehensive  Test ALL modes at 640x480 AND 640x480 (RECOMMENDED)"
                echo ""
                echo "Resolution-specific modes (NV12 - 4:2:0 default):"
                echo "  pix640     PIX mode at 640x480"
                echo "  pix1280    PIX mode at 640x480"
                ...
                echo "  rdi1280    RDI mode at 640x480 (RAW8 Bayer via RAW-through-PIX)"
                echo "  rdi1280-raw10 RDI mode at 640x480 (RAW10 Bayer via RAW-through-PIX)"
                echo "  rawpix640  RAW via PIX node at 640x480 (sensor RAW, DEMUX disabled)"
                echo "  rawpix1280 RAW via PIX node at 640x480 (sensor RAW, DEMUX disabled)"
                echo "  video640   VIDEO mode at 640x480"
                echo "  video1280  VIDEO mode at 640x480"
                echo "  testgen640   TESTGEN mode at 640x480"
                echo "  testgen1280  TESTGEN mode at 640x480"
                echo ""
                echo "NV16 (4:2:2) modes - full chroma height:"
                echo "  pix640-nv16     PIX mode at 640x480 (NV16)"
                echo "  pix1280-nv16    PIX mode at 640x480 (NV16)"
                echo "  video640-nv16   VIDEO mode at 640x480 (NV16)"
                echo "  video1280-nv16  VIDEO mode at 640x480 (NV16)"
                echo "  testgen640-nv16   TESTGEN mode at 640x480 (NV16)"
                echo "  testgen1280-nv16  TESTGEN mode at 640x480 (NV16)"
                echo ""
                echo "Color effects modes (sensor ISP effects):"
                echo "  pix640-mono     PIX mode at 640x480 with Black & White effect"
                echo "  pix1280-mono    PIX mode at 640x480 with Black & White effect"
                echo "  pix640-sepia    PIX mode at 640x480 with Sepia effect"
                echo "  pix1280-sepia   PIX mode at 640x480 with Sepia effect"
                echo "  pix640-negative PIX mode at 640x480 with Negative effect"
                echo "  pix1280-negative PIX mode at 640x480 with Negative effect"
                echo "  pix640-solarize PIX mode at 640x480 with Solarization effect"
                echo "  pix1280-solarize PIX mode at 640x480 with Solarization effect"
                ...
                echo "  pix+video1280   PIX + VIDEO simultaneous at 640x480"
                echo "  zsl640          ZSL (PIX preview + snapshot) at 640x480"
                echo "  zsl1280         ZSL (PIX preview + snapshot) at 640x480"
                echo ""
    ...
    case $MODE in
        ...
        convert-video640)
            ...
            ;;
        convert-video1280)
            convert_frames video 1280 1024 "/tmp/test_video_640x480.raw" NV12
            ;;
        convert-pix640)
            convert_frames pix 640 480 "/tmp/test_pix_640x480.raw" NV12
            ;;
        convert-pix1280)
            convert_frames pix 1280 1024 "/tmp/test_pix_640x480.raw" NV12
            ;;
        convert-rdi640)
        ...
        convert-testgen640)
            ...
            ;;
        convert-testgen1280)
            convert_frames testgen 1280 1024 "/tmp/test_testgen_640x480.raw" NV12
            ;;
        comprehensive)
            show_camera_info
            ensure_camera_ready
        ...
        pix+video1280)
            ...
            run_on_device "
                ...
                if [ -s /tmp/test_pix_video_pix_1280.raw ] && [ -s /tmp/test_pix_video_vid_1280.raw ]; then
                    ...
                    echo \"PASS: PIX+VIDEO 640x480 - PIX=\${PIX_SIZE}, VIDEO=\${VID_SIZE}\"

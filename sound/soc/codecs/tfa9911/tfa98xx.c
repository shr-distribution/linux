...
static unsigned int  tfa_mi2s_interface = 0; // MODIFIED by hongwei.tian, 2017-12-06,BUG-5709367
...
struct tfa98xx *tfa98xx_devices[4] = {NULL, NULL, NULL, NULL};
...
#ifdef CONFIG_SND_SOC_TFA98XX_MMI_TEST
u16 tfa_get_mi2s_interface(void)
{
			       ...
			       struct snd_ctl_elem_value *ucontrol)
{
	...
	/*TCTNB.CY - when changed profile during playback, set the corresponding pin state for receiver*/
	if (tfa98xx->pinctrl) {
		if ((tfa98xx_profile == PROFILE_RECEIVER_TOP) || (tfa98xx_profile == PROFILE_RECEIVER_BTM)) {	/*receiver mode*/
			if (pinctrl_select_state(tfa98xx->pinctrl, tfa98xx->pinctrl_state_active))
				pr_err("%s: can not set rcv_switch_active pinstate\n", __func__);
		} else {
			if (pinctrl_select_state(tfa98xx->pinctrl, tfa98xx->pinctrl_state_suspend))
...
static struct mutex tfa_fw_load_mutex;		/*lock for avoiding firmware load conflict of multi devices*/
...
static void tfa98xx_container_loaded(const struct firmware *cont, void *context)
{
	...
	if (!container) {
		...
		mutex_unlock(&tfa_fw_load_mutex);
		return;
	}
	...
	if (ret) {
		dev_err(tfa98xx->dev, "Loading cnt file failed %d, aborting\n", ret);
		mutex_unlock(&tfa_fw_load_mutex);
		return;
	}
	...
	dev_info(tfa98xx->dev, "Firmware init complete \n");
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(4,18,0)
	snd_soc_component_read(tfa98xx->codec, TFA98XX_STATUSREG, &val);
#else
						...
						struct snd_soc_dai *dai)
{
	...
	return 0;	//not add sr rule to platform for been supported already
...
static int tfa98xx_mute(struct snd_soc_dai *dai, int mute, int stream)
{
	...
	if (mute) {
		 ...
		 * are deactivated
		 */
		if (tfa98xx->pinctrl) {/*recv mode switch enabled defaultly*/
			ret = pinctrl_select_state(tfa98xx->pinctrl, tfa98xx->pinctrl_state_active);
			if (ret)
				pr_err("%s: can not set rcv_switch_active pinstate\n", __func__);
		}
	...
	//	cancel_delayed_work_sync(&tfa98xx->monitor_work);
		...
		if ((PROFILE_RECEIVER_TOP != tfa98xx_profile) && (PROFILE_RECEIVER_BTM != tfa98xx_profile) && tfa98xx->pinctrl) {/*recv mode switch disabled for other profile*/
			ret = pinctrl_select_state(tfa98xx->pinctrl, tfa98xx->pinctrl_state_suspend);
			if (ret)
				pr_err("%s: can not set rcv_switch_suspend pinstate\n", __func__);
		}
...
static void __tfa98xx_irq(struct tfa98xx *tfa98xx)
{
	...
	if (!(val & ie)) {
		 ...
		 */
#if LINUX_VERSION_CODE >= KERNEL_VERSION(4,18,0)
		snd_soc_component_read(tfa98xx->codec, TFA98XX_INTERRUPT_REG, &ireg);
#else
...
static int tfa98xx_parse_dt(struct device *dev, struct tfa98xx *tfa98xx,
	...
	tfa98xx->switch_gpio = of_get_named_gpio(np, "switch-gpio", 0);
	if (tfa98xx->switch_gpio < 0)
		dev_dbg(dev, "No switch GPIO provided, will not into receiver mode\n");
...
/*gpio config init*/
static int tfa98xx_pinctrl_init(struct tfa98xx *tfa98xx)
{
	int ret;

	tfa98xx->pinctrl = devm_pinctrl_get(&(tfa98xx->i2c->dev));
	if (IS_ERR_OR_NULL(tfa98xx->pinctrl)) {
		ret = PTR_ERR(tfa98xx->pinctrl);
		goto err_pinctrl_get;
	}
	/*init reset gpio*/
	tfa98xx->pinctrl_state_default= pinctrl_lookup_state(tfa98xx->pinctrl, "default");
	if (!IS_ERR_OR_NULL(tfa98xx->pinctrl_state_default)) {
		ret = pinctrl_select_state(tfa98xx->pinctrl, tfa98xx->pinctrl_state_default);
		if (ret)
			dev_err(&tfa98xx->i2c->dev,"%s: can not set default pinstate\n", __func__);
	} else {
		tfa98xx->pinctrl_state_default = NULL;
		dev_err(&tfa98xx->i2c->dev,"%s: can not get default pinstate\n", __func__);
	}

	tfa98xx->pinctrl_state_suspend = pinctrl_lookup_state(tfa98xx->pinctrl, "nxp98xx_spk_rcv_switch_suspend");
	if (IS_ERR_OR_NULL(tfa98xx->pinctrl_state_suspend)) {
		tfa98xx->pinctrl_state_suspend = NULL;
		dev_err(&tfa98xx->i2c->dev,"%s: can not get rcv_switch_suspend pinstate\n", __func__);
	}
	/*rev switch active default*/
	tfa98xx->pinctrl_state_active = pinctrl_lookup_state(tfa98xx->pinctrl, "nxp98xx_spk_rcv_switch_active");
	if (!IS_ERR_OR_NULL(tfa98xx->pinctrl_state_active)) {
		ret = pinctrl_select_state(tfa98xx->pinctrl, tfa98xx->pinctrl_state_active);
		if (ret)
			dev_err(&tfa98xx->i2c->dev,"%s: can not set rcv_switch_active pinstate\n", __func__);
	} else {
		tfa98xx->pinctrl_state_active = NULL;
		dev_err(&tfa98xx->i2c->dev,"%s: can not get rcv_switch_active pinstate\n", __func__);
	}

	return ret;

err_pinctrl_get:
	tfa98xx->pinctrl = NULL;
	return ret;
}

/*power init*/
static int tfa98xx_regulator_init(struct tfa98xx *tfa98xx)
{
    int rc = -1;

    tfa98xx->vdd = regulator_get(&tfa98xx->i2c->dev, "vdd");
    if (IS_ERR(tfa98xx->vdd)) {
        pr_err("%s %d get vdd error\n", __func__, __LINE__);
        goto err_get_vdd;
    }

    if (regulator_count_voltages(tfa98xx->vdd) > 0)
    {
        rc = regulator_set_voltage(tfa98xx->vdd, 1800000, 1800000);
        if (rc) {
    ...
    rc = regulator_enable(tfa98xx->vdd);
    if (rc) {
        dev_err(&tfa98xx->i2c->dev, "Regulator vdd enable failed rc=%d\n", rc);
        return rc;
    }
...
err_get_vdd:
    if (regulator_count_voltages(tfa98xx->vdd) > 0) {
        regulator_set_voltage(tfa98xx->vdd, 0, 1800000);
    }

    return rc;
}
extern int i2c_check_status_create(char *name,int value); // MODIFIED by hongwei.tian, 2017-12-08,BUG-5756605
			     ...
			     const struct i2c_device_id *id)
{
	...
	unsigned int reg;
	int ret;
	/* MODIFIED-BEGIN by hongwei.tian, 2016-08-19,BUG-2758479*/
	unsigned char i2c_ok = 0;
/* MODIFIED-BEGIN by hongwei.tian, 2017-12-06,BUG-5709367*/
#ifdef CONFIG_SND_SOC_TFA98XX_MMI_TEST
...
//		return -ENOMEM;
	...
	/* MODIFIED-END by hongwei.tian,BUG-2758479*/
	...
	tfa98xx_regulator_init(tfa98xx);
	tfa98xx_pinctrl_init(tfa98xx);
	...
	if (gpio_is_valid(tfa98xx->switch_gpio)) {
		ret = devm_gpio_request_one(&i2c->dev, tfa98xx->switch_gpio,
			GPIOF_OUT_INIT_HIGH, "TFA98XX_SWITCH");
		if (ret)
			goto err;
	}
	...
	if (no_start == 0) {
		...
		if (ret < 0) {
			dev_err(&i2c->dev, "Failed to read Revision register: %d add=%x\n",
				ret, i2c->addr);
			/* MODIFIED-BEGIN by hongwei.tian, 2016-08-19,BUG-2758479*/
			ret = -EIO;
			goto err;
...
/*init lock once for avoiding firmware load conflict of multi devices*/
	mutex_init(&tfa_fw_load_mutex);
...
err:
	i2c_check_status_create("audio_smpart_pa",0);
...
//	cancel_delayed_work_sync(&tfa98xx->monitor_work);
	...
	if (gpio_is_valid(tfa98xx->switch_gpio))
		devm_gpio_free(&i2c->dev, tfa98xx->switch_gpio);

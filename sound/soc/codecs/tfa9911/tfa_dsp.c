...
const char* tfa98xx_get_i2c_status_id_string(int status)
{
        const char* p_id_str;
        /*
         * The default case below sprintf()s into this buffer and returns
         * a pointer to it, so it has to outlive the function. It used to
         * be on the stack, making every "Unspecified error" return a
         * dangling pointer (GCC 15: -Werror=return-local-addr).
         */
...
enum tfa_error tfa_start(int next_profile, int *vstep)
{
	...
	for( dev=0; dev < devcount; dev++) {
		...
		if (( next_profile != active_profile && active_profile != -1)	/*update driver from version r3 to r4*/
            || (istap_prof == 1)) {

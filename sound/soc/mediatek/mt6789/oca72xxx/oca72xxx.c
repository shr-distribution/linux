				...
				const struct i2c_device_id *id)
{
	...
	if (!i2c_check_functionality(client->adapter, I2C_FUNC_I2C)) {
		...
		printk("moshaoxi oca72xxx_i2c_probe error--check_functionality\n");
		ret = -ENODEV;
		goto exit_check_functionality_failed;
	}

	printk("moshaoxi oca72xxx_i2c_probe enter ok\n");
	...
	OCA_DEV_LOGE(oca72xxx->dev, "succeed, dev_index=[%d], g_oca72xxx_dev_cnt= [%d]",
			oca72xxx->dev_index, g_oca72xxx_dev_cnt);


	printk("moshaoxi oca72xxx_i2c_probe exit\n");

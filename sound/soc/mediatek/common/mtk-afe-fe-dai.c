...
MEM_ALLOCATE_DONE:
	...
	/*only mt6983 need 16bit etdm feature*/
	if (of_property_read_string(afe->dev->of_node, "compatible", &compatible)) {
		dev_info(afe->dev, "%s(), Failed to read compatible property\n",
		__func__);
		return -EINVAL;
	}
	if (strcmp(compatible, "mediatek,mt6983-sound") == 0) {
		dev_info(afe->dev, "%s(), compatible = %s, enable 16bit etdm\n",
		__func__, compatible);

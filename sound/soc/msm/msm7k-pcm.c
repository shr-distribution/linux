...
int copy_count;
int playback_instance = 0;
int capture_instance = 0;
...
//	pr_info("%s:prtd->out_tail =%d mmap_flag=%d\n", __func__, prtd->out_tail, prtd->mmap_flag);
...
static int msm_pcm_playback_prepare(struct snd_pcm_substream *substream)
{
	...
	printk(KERN_INFO "+playback_prepare\n");
...
static int msm_pcm_trigger(struct snd_pcm_substream *substream, int cmd)
{
	...
	printk(KERN_INFO"msm_pcm_trigger\n");
	switch (cmd) {
	...
	case SNDRV_PCM_TRIGGER_PAUSE_RELEASE:
		if (substream->stream == SNDRV_PCM_STREAM_CAPTURE)
			break;
		if (!prtd->out_needed | !prtd->mmap_flag) {
			prtd->stopped = 0;
			break;
		}
...
//	pr_info("%s:prtd->pcm_buf_pos =%d, prtd->mmap_flag =%d\n", __func__, prtd->pcm_buf_pos, prtd->mmap_flag);
	return rc;
}
...
static int msm_pcm_capture_close(struct snd_pcm_substream *substream)
{
	...
	printk(KERN_INFO"+msm_capture_close\n");
				...
				struct vm_area_struct *vma)
{
	...
	dma_mmap_coherent(substream->pcm->card->dev, vma,
				     ...
				     runtime->dma_addr,
				     runtime->dma_bytes);
	return 0;//alsa_adsp_configure(prtd);
}

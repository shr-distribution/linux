...
int alsa_adsp_configure(struct msm_audio *prtd)
{
	...
	if (prtd->dir == SNDRV_PCM_STREAM_PLAYBACK) {
		if(prtd->mmap_flag)
			prtd->out_buffer_size =
				prtd->playback_substream->runtime->dma_bytes;
		else
		...
		prtd->out[0].size = prtd->out_buffer_size/2;
		prtd->out[1].data = prtd->data + prtd->out_buffer_size/2;
		prtd->out[1].addr = prtd->phys + prtd->out_buffer_size/2;
		prtd->out[1].size = prtd->out_buffer_size/2;
	}
	if (prtd->dir == SNDRV_PCM_STREAM_CAPTURE) {
...
//	printk(KERN_INFO"+audio_send_buffer\n");
...
EXPORT_SYMBOL(audio_send_buffer);
	...
	while (count > 0) {
		...
		if (count >= size) {
			...
			if (index != prtd->in_tail) {
				pr_info("alsa: overrun: data is invalid, we need to retry\n");
...
void alsa_get_dsp_frames(struct msm_audio *prtd)
{
	...
	if (prtd->type == AUDREC_CMD_TYPE_0_INDEX_WAV) {
		...
		if (prtd->in_head == prtd->in_tail) {
			pr_info("alsa: Overrun: Moving the tail index\n");
			prtd->in_tail = (prtd->in_tail + 1) & (FRAME_NUM - 1);
		} else
			prtd->in_count++;
